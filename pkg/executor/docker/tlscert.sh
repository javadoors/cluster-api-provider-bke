...
openssl req -new -x509 -days 36500 -key ca-key.pem -sha256 -out ca.pem -subj $SUBJ
...
# 10.使用根证书签发客户端证书
openssl x509 -req -days 36500 -sha256 -in client.csr -CA ca.pem -CAkey ca-key.pem -CAcreateserial -out cert.pem -extfile extfile.cnf
