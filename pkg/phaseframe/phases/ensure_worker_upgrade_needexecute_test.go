...
func workerUpgradeTestCluster(specK8s, statusK8s string) *bkev1beta1.BKECluster {
...
func newWorkerUpgradePhase(t *testing.T, cluster *bkev1beta1.BKECluster, vc *upgrade.VersionContext) *EnsureWorkerUpgrade {
	t.Helper()
	...
	return NewEnsureWorkerUpgrade(pc).(*EnsureWorkerUpgrade)
}

func TestEnsureWorkerUpgrade_isKubernetesWorkerNeedUpgrade(t *testing.T) {
	tests := []struct {
		name    string
		...
		{name: "empty target", cluster: workerUpgradeTestCluster("", "v1.28.0"), want: false},
		{name: "empty current", cluster: workerUpgradeTestCluster("v1.29.0", ""), want: false},
		{name: "spec differs from status", cluster: workerUpgradeTestCluster("v1.29.0", "v1.28.0"), want: true},
		{name: "spec matches status", cluster: workerUpgradeTestCluster("v1.28.0", "v1.28.0"), want: false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			phase := &EnsureWorkerUpgrade{}
			if tt.cluster != nil {
				phase = newWorkerUpgradePhase(t, tt.cluster, nil)
			}
			assert.Equal(t, tt.want, phase.isKubernetesWorkerNeedUpgrade(nil, tt.cluster))
		})
	}
}

func TestEnsureWorkerUpgrade_NeedExecute_VersionContext(t *testing.T) {
	cluster := workerUpgradeTestCluster("v1.29.0", "v1.28.0")
	...
	t.Run("version context needs upgrade", func(t *testing.T) {
		...
		phase := newWorkerUpgradePhase(t, cluster, vc)
		...
		patches.ApplyFunc(phaseutil.GetNeedUpgradeWorkerNodesWithBKENodes, func(_ *bkev1beta1.BKECluster, _ bkev1beta1.BKENodes) bkenode.Nodes {
			return bkenode.Nodes{{IP: "10.0.0.2"}}
		})
	...
	t.Run("version context already at target", func(t *testing.T) {
		vc := upgrade.NewVersionContext()
		vc.SetCurrent(upgrade.ComponentKubernetesWorker, "v1.28.0")
		vc.SetTarget(upgrade.ComponentKubernetesWorker, "v1.28.0")
	...
	t.Run("legacy path when version context undecided", func(t *testing.T) {
		phase := newWorkerUpgradePhase(t, cluster, nil)
		patches := gomonkey.NewPatches()
		defer patches.Reset()
		patches.ApplyFunc(fetchBKENodesIfCPInitialized, func(_ *phaseframe.PhaseContext, _ *bkev1beta1.BKECluster) (bkev1beta1.BKENodes, bool) {
			return bkev1beta1.BKENodes{}, true
	...
	t.Run("legacy path no upgrade when versions match", func(t *testing.T) {
		matched := workerUpgradeTestCluster("v1.28.0", "v1.28.0")
		phase := newWorkerUpgradePhase(t, matched, nil)
