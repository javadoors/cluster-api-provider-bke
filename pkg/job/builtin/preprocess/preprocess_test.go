...
func TestLoadConfigPriority(t *testing.T) {
	...
	p := &PreprocessPlugin{k8sClient: fakeClient}
...
func TestLoadConfigBatchAndNode(t *testing.T) {
	...
	batchCM := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "preprocess-config-batch-002",
			Namespace: "user-system",
		},
		Data: map[string]string{
...
func TestLoadConfigNodeHit(t *testing.T) {
	...
	nodeCM := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "preprocess-config-node-10.0.0.2",
			Namespace: "user-system",
		},
		Data: map[string]string{
			"config.json": `{"scripts":[{"scriptName":"node.sh","order":1}]}`,
...
func TestGetAllScriptsAndScriptExists(t *testing.T) {
	cm1 := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "script-a.sh",
			Namespace: "user-system",
			Labels:    map[string]string{"bke.preprocess.script": "true"},
		},
	}
...
func TestExecuteScriptParamValidationSkip(t *testing.T) {
	...
	p := &PreprocessPlugin{k8sClient: fakeClient, exec: &fakeExecutor{}}
...
func TestLoadConfigParseError(t *testing.T) {
	...
	_, err := p.loadConfig("10.0.0.1")
	require.Error(t, err)
}

func TestGetAllScriptsEmpty(t *testing.T) {
	fakeClient, _ := testutils.TestGetRuntimeFakeClient(nil)
	p := &PreprocessPlugin{k8sClient: fakeClient}

	scripts, err := p.getAllScripts()
	require.NoError(t, err)
	require.Len(t, scripts, 0)
}
...
func TestParseConfigSuccess(t *testing.T) {
	...
	cm := &corev1.ConfigMap{
		...
		Data: map[string]string{
			"config.json": `{"scripts":[{"scriptName":"a.sh","order":1,"params":{"ROLE":"master"}}]}`,
		},
	}
	cfg, err := p.parseConfig(cm)
	require.NoError(t, err)
	require.Len(t, cfg.Scripts, 1)
	require.Equal(t, "a.sh", cfg.Scripts[0].ScriptName)
}

func TestLoadConfigBatchMissingMapping(t *testing.T) {
	batchMapping := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "preprocess-node-batch-mapping",
			Namespace: "user-system",
		...
		Data: map[string]string{
			"mapping.json": `{"10.0.0.2":"001"}`,
		},
	}
	fakeClient, _ := testutils.TestGetRuntimeFakeClient(nil, batchMapping)
	p := &PreprocessPlugin{k8sClient: fakeClient}
...
func TestLoadConfigBatchMappingParseError(t *testing.T) {
	batchMapping := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "preprocess-node-batch-mapping",
			Namespace: "user-system",
...
func TestLoadConfigBatchConfigMissing(t *testing.T) {
	batchMapping := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "preprocess-node-batch-mapping",
			Namespace: "user-system",
...
func TestLoadConfigNodeConfigMissing(t *testing.T) {
	fakeClient, _ := testutils.TestGetRuntimeFakeClient(nil)
	p := &PreprocessPlugin{k8sClient: fakeClient}

	_, err := p.loadConfig("10.0.0.9")
	require.Error(t, err)
}

func TestGetAllScriptsWithNamespace(t *testing.T) {
	cm := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "script-c.sh",
			Namespace: "user-system",
	...
	scripts, err := p.getAllScripts()
	require.NoError(t, err)
	require.Equal(t, []string{"script-c.sh"}, scripts)
}
