...
| 序号 | 特性 | 原始(天) | 悲观评估(人月) | 说明 |
|------|------|---------|---------------|------|
| R1 | PhaseFlow 回滚流程设计与评审 | - | **0.5** | 需求分析、方案设计、技术评审 |
| R2 | PhaseFlow 回滚触发机制开发 | 5 | **0.8** | 手动/自动触发、CLI 命令、注解处理 |
| R3 | PhaseFlow 回滚状态转换规则开发 | 8 | **1.2** | 状态机扩展、转换规则、兼容性验证 |
| R4 | PhaseFlow 资源清理逻辑开发 | 5 | **1.0** | 扩缩容/配置变更/删除的清理逻辑 |
| R5 | PhaseFlow 回滚验证与测试 | 5 | **0.8** | 单元测试、集成测试、端到端测试 |
| R6 | 安装失败清理脚本 | 5 | **0.8** | 脚本开发、多环境测试 |
| R7 | ClusterVersion 版本回滚方案设计 | - | **0.5** | 方案二设计、兼容性分析 |
| R8 | ClusterVersion 版本回滚逻辑开发 | 5 | **1.0** | 重新部署旧版本、版本验证 |
| R9 | ClusterVersion 回滚路径验证开发 | 5 | **0.8** | UpgradePath CRD 扩展、路径验证 |
| R10 | ClusterVersion 降级 DAG 设计 | - | **0.8** | 反向拓扑排序、组件依赖分析 |
| R11 | ClusterVersion 降级 DAG 开发 | 18 | **2.5** | DAG 编排器、反向执行逻辑 |
| R12 | 组件降级逻辑开发 | - | **1.5** | etcd/Agent/Containerd/Master/Worker 降级 |
| R13 | ClusterVersion 回滚历史管理 | 3 | **0.5** | UpgradeHistory 扩展、查询接口 |
| | **回滚能力小计** | | **11.7** | |
...
| 序号 | 特性 | 原始(天) | 悲观评估(人月) | 说明 |
|------|------|---------|---------------|------|
| B1 | etcd 自动备份 | 5 | **0.5** | 定时任务、清理、监控 |
| B2 | 配置与证书备份 | 3 | **0.3** | 备份脚本、验证 |
| B3 | 应用数据备份（Velero） | 5 | **0.8** | Velero 集成、配置 |
| B4 | PVC 快照备份 | 3 | **0.5** | SnapshotClass、测试 |
| B5 | 备份存储与加密 | 3 | **0.5** | 多存储后端、加密 |
| | **备份能力小计** | | **2.6** | |
...
| 序号 | 特性 | 原始(天) | 悲观评估(人月) | 说明 |
|------|------|---------|---------------|------|
| C1 | etcd 恢复 | 3 | **0.5** | 恢复脚本、验证 |
| C2 | 配置恢复 | 2 | **0.3** | 恢复脚本 |
| C3 | 应用数据恢复 | 2 | **0.3** | Velero 恢复 |
| C4 | 灾难恢复流程设计 | - | **0.3** | 灾难场景分析、恢复流程设计 |
| C5 | 灾难恢复流程开发 | 8 | **1.2** | 完整流程、自动化脚本 |
| C6 | 恢复验证开发 | 5 | **0.8** | 自动化验证、健康检查 |
...
| 组件 | 回滚方案 | 复杂度 | 工作量 | 关键挑战 |
|------|---------|--------|--------|---------|
| **BKE Agent** | SSH 重新推送旧版本二进制 + 重启服务 | 简单 | 2-3天 | SSH 连通性；服务状态清理 |
| **证书和密钥** | 从备份恢复 PKI 目录 | 简单 | 1-2天 | 证书链验证；信任存储更新 |
| **Containerd** | 重置 + 重新部署旧版本（复用现有逻辑） | 中等 | 3-5天 | 需要驱逐容器；镜像缓存失效 |
| **Master 组件** | 停止静态 Pod → 替换二进制/清单 → 重启 | 中等 | 5-7天 | API Server 可用性；证书兼容性 |
| **Worker 组件** | 驱逐 Pod → 停止 kubelet → 替换二进制 → 重启 | 中等 | 3-5天 | Pod 驱逐；节点可用性 |
| **etcd** | 恢复快照 + 降级二进制 | **复杂** | 7-10天 | 数据格式兼容性；集群仲裁 |
| **配置** | 从备份恢复 ConfigMap/Secret + 重新应用本地配置 | 中等 | 3-5天 | 配置漂移；Schema 兼容性 |
| **DAG 编排器** | 反向拓扑排序执行 | **复杂** | 5-7天 | 反向执行逻辑；错误处理 |
...
- 悲观系数：原始工作量 × 1.5（考虑需求变更、技术难点、调试时间）
- 隐性工作量：集成测试、文档、代码审查、联调（额外增加 20%）

#### 6.3.1 里程碑总览

| 季度 | 里程碑 | 工作量(人月) | 团队利用率 | 核心交付 |
|------|--------|-------------|-----------|---------|
| **2026Q4** | M1: 基础备份与安装清理 | **3.0** | 25% | etcd 备份/恢复、配置备份、安装清理 |
| **2027Q1** | M2: PhaseFlow 回滚 | **4.5** | 38% | 回滚触发、状态转换、资源清理、回滚验证 |
| **2027Q2** | M3: 版本回滚 + 应用备份 | **4.0** | 33% | 版本回滚（方案二）、Velero 集成、PVC 快照 |
| **2027Q3** | M4: 降级 DAG + 灾难恢复 | **6.0** | 50% | 降级 DAG（方案一）、灾难恢复流程、恢复验证 |
| **总计** | | **17.5** | **37%** | |

#### 6.3.2 2026Q4（M1）：基础备份与安装清理（3.0 人月）
...
#### 6.3.3 2027Q1（M2）：PhaseFlow 回滚能力（4.5 人月）
...
#### 6.3.4 2027Q2（M3）：版本回滚 + 应用备份（4.0 人月）
...
#### 6.3.5 2027Q3（M4）：降级 DAG + 灾难恢复（6.0 人月）

| 特性 | 人月 | 交付物 | 验收标准 |
|------|------|--------|---------|
| R10: 降级 DAG 设计 | 0.8 | 设计文档、依赖分析 | 方案评审通过 |
| R11: 降级 DAG 开发 | 2.5 | 降级 DAG、逐组件降级逻辑 | 复杂场景可降级 |
