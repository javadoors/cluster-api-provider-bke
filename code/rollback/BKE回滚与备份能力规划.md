...
| 版本类型 | 发布周期 | 支持周期 | 说明 |
|---------|---------|---------|------|
| 标准版本 | 每季度 | 短期支持（至下一个 LTS） | 适合追求新特性的用户 |
| LTS 版本 | 每年12月 | 长期支持（12个月以上） | 适合追求稳定性的生产环境 |
...
|------|--------|-------------|-----------|---------|
| **2026Q4** | M1: 基础备份与安装清理 | **4.0** | 33% | etcd 备份/恢复、配置备份、安装清理 |
| **2027Q1** | M2: PhaseFlow 回滚 + 升级验证 | **5.5** | 46% | 回滚触发、状态转换、资源清理、升级前后验证 |
| **2027Q2** | M3: 版本回滚 + 升级路径增强 | **5.3** | 44% | 版本回滚（方案二）、UpgradePath CRD 增强、停机窗口优化 |
| **2027Q3** | M4: 降级 DAG + 灾难恢复 | **7.0** | 58% | 降级 DAG（方案一）、灾难恢复流程、恢复验证 |
| **总计** | | **21.8** | **45%** | |

#### 7.4.2 2026Q4（M1）：基础备份与安装清理（4.0 人月）
...
#### 7.4.5 2027Q3（M4）：降级 DAG + 灾难恢复（7.0 人月）
...
**说明**：
- 总工作量 **21.4 人月**，分散在 4 个季度
- 团队利用率约 **45%**，留有充足缓冲处理突发问题
- 回滚能力占比最高（54%），是核心能力
- 每个里程碑都有明确的交付物和验收标准
