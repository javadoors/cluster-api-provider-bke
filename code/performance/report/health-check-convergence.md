...
本提案旨在优化 BKE 集群创建过程中的健康检查收敛时间，将其从当前的 7 分 14 秒缩短至 < 1 分钟（提升 86%）。
...
| 指标 | 当前值 | 目标值 | 提升 |
| ------ | -------- | -------- | ------ |
| 健康检查收敛时间 | 7 分 14 秒 | < 1 分钟 | 86% |
| ClusterUnhealthy 次数 | 33 次 | < 5 次 | 85% |
| Master NotReady 次数 | 3 次 | 0 次 | 100% |
| API 调用次数 | ~100 次/检查 | < 10 次/检查 | 90% |
| 关键组件失败检测时间 | ~7 分钟 | < 30 秒 | 99% |
...
1. 健康检查收敛时间：从 7 分 14 秒缩短至 < 1 分钟（提升 86%）
2. Master NotReady 次数：从 3 次减少至 0 次
3. API 调用次数：从 ~100 次/检查降低至 < 10 次/检查（降低 90%）
4. 关键组件失败检测时间：从 ~7 分钟缩短至 < 30 秒
5. ClusterUnhealthy 次数：从 33 次减少至 < 5 次（减少 85%）
...
- [ ] 集成测试通过
- [ ] 健康检查收敛时间 < 2 分钟（相比 7 分 14 秒提升 72%）
- [ ] API 调用次数 < 10 次（首次同步后）
- [ ] Master NotReady 次数 < 1 次
- [ ] 配置文件支持完整
...
| 指标 | 当前值 | 目标值 | 验收标准 |
| ------ | -------- | -------- | ---------- |
| 健康检查收敛时间 | 7 分 14 秒 | < 1 分钟（提升 86%） | 64 节点集群端到端测试 |
| API 调用次数 | ~100 次/检查 | < 10 次/检查 | Informer 首次同步后 |
| Master NotReady 次数 | 3 次 | 0 次 | 完整集群创建周期无 NotReady 事件 |
| 关键组件失败检测时间 | ~7 分钟 | < 30 秒 | 关键组件失败立即返回 |
| ClusterUnhealthy 次数 | 33 次 | < 5 次 | 完整集群创建周期 |
...
| 健康检查时间 | < 2 分钟（相比 7 分 14 秒提升 72%） | 64 节点集群测试 |
| API 调用次数 | < 10 次（首次同步后） | 监控指标验证 |
| Master NotReady 次数 | < 1 次 | 日志分析 |
