...
本文档共 5500+ 行，为帮助您快速定位所需信息，按使用场景提供以下导航：

### 设计评审人员

如果您需要评审本设计方案，建议按以下顺序阅读：

1. **[1. 摘要](#1-摘要)** - 了解整体方案和目标（5分钟）
2. **[2. 动机](#2-动机)** - 理解现有问题和改进动机（15分钟）
3. **[3. 范围与约束](#3-范围与约束)** - 明确方案边界（10分钟）
4. **[4. 提案设计](#4-提案设计)** - 详细设计方案（30分钟）
5. **[5. 综合重构方案](#5-综合重构方案)** - 实施计划和演进路径（15分钟）
6. **[附录 A. 术语表](#a-术语表)** - 统一概念理解（10分钟）

### 实施开发人员

如果您需要实施本方案，建议按以下顺序阅读：

1. **[1. 摘要](#1-摘要)** - 了解整体方案（5分钟）
...
如果您需要验证本方案，建议按以下顺序阅读：

1. **[1. 摘要](#1-摘要)** - 了解整体方案（5分钟）
2. **[7. 测试策略](#7-测试策略)** - 单元测试和集成测试（20分钟）
3. **[4.2 增强方案一：状态转换表](#42-增强方案一状态转换表适配单字段设计)** - 状态转换规则（30分钟）
4. **[4.3 增强方案二：改进状态管理器](#43-增强方案二改进状态管理器适配单字段设计)** - 重试机制（20分钟）
5. **[9. 风险管理](#9-风险管理)** - 回滚方案和灰度策略（15分钟）

### 运维操作人员

如果您需要运维本方案，建议按以下顺序阅读：

1. **[1. 摘要](#1-摘要)** - 了解整体方案（5分钟）
2. **[6. 迁移策略](#6-迁移策略)** - 向后兼容策略（15分钟）
3. **[9. 风险管理](#9-风险管理)** - 回滚方案、灰度策略、监控告警（20分钟）
4. **[4.4 增强方案三：状态转换事件系统](#44-增强方案三状态转换事件系统适配单字段设计)** - 事件记录和查询（15分钟）

### 关键概念快速查找

| 概念 | 章节位置 | 说明 |
|------|---------|------|
| 三字段整合 | [4.1](#41-重构方案三字段整合方案保持-clusterstatus-兼容性) | Phase、ClusterStatus、ClusterHealthState 统一为 ClusterStatus |
...
// NewEngine 创建状态机引擎
func NewEngine(client client.Client, ctx context.Context) *Engine {
    return &Engine{
...
// recordTransition 记录转换事件（基础版：内存列表，用于开发调试）
// 生产环境可通过 2.4 节的 EventStore 接口替换为增强版（支持持久化/查询/导出）
...
步骤 1 (1天): 新增 statemachine 包
  ├── engine.go: Engine + Transition 方法
