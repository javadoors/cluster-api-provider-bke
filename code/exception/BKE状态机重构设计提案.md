...
|-----------|------------|---------|------|
| **状态转换逻辑分散**：28 个状态转换点分布在 6 个文件中，11 个独立的 `handleCluster*Phase` 函数 | 4.2 节（状态转换表） | 使用状态转换表，集中管理所有转换规则，删除 11 个分散的函数 | ✅ 已解决 |
...
|-----------|------------|---------|------|
| **三字段职责重叠**：`Phase`、`ClusterStatus`、`ClusterHealthState` 语义重叠率高达 50%-56% | 4.1 节（三字段整合） | 以 `ClusterStatus` 为单一数据源，`Phase` 和 `ClusterHealthState` 标记为 Deprecated | ✅ 已解决 |
| **状态管理器内存泄漏**：全局单例无清理机制 | 4.3 节（改进状态管理器） | 新增 `StatusCleaner`，自动清理过期记录（24 小时过期） | ✅ 已解决 |
| **固定重试次数**：全局固定 10 次重试 | 4.3 节（改进状态管理器） | 按 `ClusterStatus` 索引，支持不同重试策略和退避策略 | ✅ 已解决 |
...
|---------|---------|------------|---------|------|
| 2.3 | **Phase 状态管理问题**：Phase 历史记录限制、执行顺序硬编码 | 4.1 节（三字段整合） | 使用 `ClusterStatus` 替代 `Phase`，提供映射函数 | ✅ 已解决 |
| 2.4 | **并发安全问题**：潜在的死锁风险、竞态条件 | 4.3 节（改进状态管理器） | 使用 `sync.RWMutex` 和原子操作，确保并发安全 | ✅ 已解决 |
...
- **4.2 节（状态转换表）**：解决"状态如何转换"的问题
- **4.3 节（改进状态管理器）**：解决内存泄漏、灵活重试、并发安全等问题
- **4.4 节（状态转换事件系统）**：提供可观测性支持
