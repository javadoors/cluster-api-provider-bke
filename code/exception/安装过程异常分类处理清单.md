...
└─────────────────────────────────────────────────────────────────────────────┘
  ...
  开始安装
     ...
     ▼
┌─────────────────┐
│ EnsureCerts     │── 失败 ──▶ 自动重试 (无限制) ──▶ 持续直到成功
│ 证书生成         │── 成功 ──▶ 继续
└────────┬────────┘
         ▼
┌─────────────────┐
│ EnsureBKEAgent  │── 失败 ──▶ 单节点重试3次 ──▶ 全部失败 ▶ Requeue
│ Agent推送        │── 部分失败 ──▶ 标记失败节点，继续 ──▶ 下次协调重试
│                 │── 成功 ──▶ 继续
└────────┬────────┘
         ▼
┌─────────────────┐
│ EnsureNodesEnv  │── 失败 ──▶ Command重试(BackoffLimit) ──▶ 超时 ▶ Requeue
│ 节点环境初始化    │── 部分失败 ──▶ 标记失败节点，返回错误
│                 │── 成功 ──▶ 继续
└────────┬────────┘
         ▼
┌─────────────────┐
│ EnsureCluster   │── 失败 ──▶ 自动重试 (无限制) ──▶ 持续直到成功
│ APIObj          │── 成功 ──▶ 继续
└────────┬────────┘
         ▼
┌─────────────────┐     ┌──────────────────┐
...
```mermaid
graph TD
    Start[开始安装] --> Certs{EnsureCerts<br/>证书生成}
    Certs -->|失败| CertsRetry[自动重试 无限制]
    CertsRetry --> Certs
    Certs -->|成功| Agent{EnsureBKEAgent<br/>Agent推送}
    ...
    CAObj -->|失败| CAObjRetry[自动重试 无限制]
    CAObjRetry --> CAObj
    CAObj -->|成功| MasterInit{EnsureMasterInit<br/>Master初始化}
    ...
    WorkerJoin -->|成功| PostProcess{EnsureNodesPostProcess<br/>后置处理}
    WorkerJoinPartial --> PostProcess
    WorkerJoinFail --> PostProcess
    
    PostProcess -->|失败| PostIgnore[忽略错误 继续]
    PostIgnore --> Done[安装完成<br/>ClusterStatus=Ready]
    PostProcess -->|成功| Done
