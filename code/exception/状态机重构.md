...
| 任务 | 内容 | 预计时间 | 验收标准 |
|------|------|----------|----------|
| **任务 1：API 层** | 为 `Phase` 和 `ClusterHealthState` 字段添加 Deprecated 注释 | 0.5 天 | 字段注释已更新 |
| **任务 2：映射函数** | 新增 4 个映射函数及单元测试 | 1 天 | 映射函数单元测试通过 |
| **任务 3：PhaseFlow 框架** | 修改 PhaseFlow 框架，使用 ClusterStatus，同步设置旧字段 | 2 天 | 所有使用 Phase 的代码已修改 |
| **任务 4：状态管理层** | 修改状态管理层，使用 ClusterStatus，同步设置旧字段 | 1.5 天 | 所有使用 ClusterHealthState 的代码已修改 |
| **任务 5：控制器和 Webhook** | 修改控制器和 Webhook，使用 ClusterStatus，同步设置旧字段 | 1 天 | 所有测试通过 |
...
| **提高代码质量** | 统一状态字段，简化代码逻辑 | 代码更清晰，维护更容易 |
...
**本阶段实施完成后（至少 6 个月后）**，可以考虑删除 `Phase` 和 `ClusterHealthState` 字段，但需要满足以下前提条件：

**前提条件**：
1. 所有外部消费者已完成迁移，不再使用旧字段
2. 已发布迁移指南和迁移工具
3. 已在文档中提前至少 6 个月通知字段删除计划
4. 已获得相关团队的确认和同意
...
| 阶段 | 内容 | 预计时间 | 前提条件 |
|------|------|----------|----------|
| **阶段 2：通知外部消费者** | 发布迁移指南，提供迁移工具 | 1-2 周 | 本阶段实施完成 |
| **阶段 3：删除字段和映射函数** | 删除 `Phase` 和 `ClusterHealthState` 字段，删除映射函数 | 1-2 天 | 所有外部消费者已完成迁移 |

**注意事项**：
- **不要提前删除字段**：删除字段是破坏性变更，必须确保所有外部消费者已完成迁移
- **映射函数在本阶段保留**：映射函数在本阶段用于内部代码迁移和向后兼容，不要删除
- **提供充足的迁移时间**：至少给外部消费者 6 个月的迁移时间
