...
| 步骤 | 操作 | 文件 | 预计时间 |
|------|------|------|----------|
| 1.1 | 创建映射函数文件 | `pkg/phaseframe/mapper.go` | 0.5 天 |
| 1.2 | 实现 `MapPhaseToClusterStatus` 函数 | `pkg/phaseframe/mapper.go` | 0.5 天 |
| 1.3 | 实现 `MapToV3LifecyclePhase` 函数 | `pkg/phaseframe/mapper.go` | 0.5 天 |
| 1.4 | 编写映射函数单元测试 | `pkg/phaseframe/mapper_test.go` | 0.5 天 |
| 1.5 | 更新文档 | `code/specification/升级规格.md` | 0.5 天 |

**第二阶段：删除 Phase 字段（3-4 天）**

| 步骤 | 操作 | 文件 | 修改处数 | 预计时间 |
|------|------|------|----------|----------|
| 2.1 | 修改 `handleRunningStatus` 方法 | `pkg/phaseframe/base.go` | 1 处 | 0.5 天 |
| 2.2 | 修改使用 Phase 字段的代码 | `pkg/phaseframe/phases/ensure_*.go` | 32 处 | 1.5 天 |
| 2.3 | 修改控制器代码 | `controllers/capbke/bkecluster_controller.go` | 2 处 | 0.5 天 |
| 2.4 | 运行单元测试 | - | - | 0.5 天 |
| 2.5 | 运行集成测试 | - | - | 0.5 天 |
| 2.6 | 修复发现的问题 | - | - | 0.5 天 |
...
| 步骤 | 操作 | 文件 | 修改处数 | 预计时间 |
|------|------|------|----------|----------|
| 3.1 | 修改状态管理器代码 | `pkg/statusmanage/statusmanager.go` | 5 处 | 1 天 |
| 3.2 | 修改其他使用代码 | 其他文件 | 约 17 处 | 1 天 |
| 3.3 | 运行单元测试 | - | - | 0.5 天 |
| 3.4 | 运行集成测试 | - | - | 0.5 天 |
| 3.5 | 修复发现的问题 | - | - | 0.5 天 |
...
| 步骤 | 操作 | 文件 | 预计时间 |
|------|------|------|----------|
| 4.1 | 删除 `Phase` 字段定义 | `api/bkecommon/v1beta1/bkecluster_status.go` | 0.5 天 |
| 4.2 | 删除 `ClusterHealthState` 字段定义 | `api/bkecommon/v1beta1/bkecluster_status.go` | 0.5 天 |
| 4.3 | 删除 `ClusterHealthState` 枚举 | `api/capbke/v1beta1/bkecluster_consts.go` | 0.5 天 |
| 4.4 | 更新测试用例 | `*_test.go` | 0.5 天 |
| 4.5 | 更新文档 | `code/specification/升级规格.md` | 0.5 天 |
| 4.6 | 代码审查 | - | 0.5 天 |
...
**示例 2：ClusterHealthState 字段迁移**

旧代码：
```go
// statusmanager.go:206
switch sr.CurrentClusterState {
case bkev1beta1.Deploying:
    bkeCluster.Status.ClusterHealthState = bkev1beta1.DeployFailed
...
```go
// statusmanager.go:206
// 删除 ClusterHealthState 的设置，只设置 ClusterStatus
switch sr.CurrentClusterState {
...
旧代码：
```go
// bkecluster_controller.go:757
if bkeCluster.Status.ClusterHealthState == bkev1beta1.Deploying {
    // 执行部署逻辑
}
```
...
```go
// bkecluster_controller.go:757
if bkeCluster.Status.ClusterStatus == bkev1beta1.ClusterDeployingAddon {
    // 执行部署逻辑
}
```
...
| 测试项 | 测试内容 | 预期覆盖率 | 预计时间 |
|--------|----------|------------|----------|
| `MapPhaseToClusterStatus` | 测试所有 Phase 到 ClusterStatus 的映射 | 100% | 0.5 天 |
| `MapToV3LifecyclePhase` | 测试所有 ClusterStatus 到 v3 LifecyclePhase 的映射 | 100% | 0.5 天 |
| 删除字段后的代码 | 测试删除 Phase 和 ClusterHealthState 后的代码逻辑 | 90% | 1 天 |

**集成测试**

| 测试场景 | 测试内容 | 预期结果 | 预计时间 |
|----------|----------|----------|----------|
| 集群创建 | 完整的集群创建流程 | 所有状态正确转换 | 0.5 天 |
| 集群升级 | 完整的集群升级流程 | 所有状态正确转换 | 0.5 天 |
| 集群删除 | 完整的集群删除流程 | 所有状态正确转换 | 0.5 天 |
| 失败重试 | 失败后的重试机制 | 重试逻辑正确 | 0.5 天 |

**端到端测试**

| 测试环境 | 测试内容 | 预期结果 | 预计时间 |
|----------|----------|----------|----------|
| 测试环境 | 部署完整的 BKE 系统 | 系统正常运行 | 1 天 |
| 生产环境（灰度） | 在灰度环境部署 | 无问题后全量发布 | 2 天 |
...
| 条件 | 阈值 | 说明 |
|------|------|------|
| 单元测试失败率 | > 10% | 单元测试失败比例过高 |
| 集成测试失败率 | > 5% | 集成测试失败比例过高 |
| 端到端测试 | 发现严重问题 | 系统无法正常运行 |
| 性能下降 | > 20% | 性能显著下降 |
...
| 验证项 | 验证方法 | 预期结果 |
|--------|----------|----------|
| 单元测试 | 运行完整的单元测试套件 | 100% 通过 |
| 集成测试 | 运行完整的集成测试套件 | 100% 通过 |
| 系统功能 | 执行核心功能测试 | 所有功能正常 |
| 性能指标 | 对比回滚前后的性能指标 | 性能恢复正常 |
...
| 兼容性类型 | 保证措施 | 说明 |
|------------|----------|------|
| ClusterStatus 字段 | 保持不变 | 字段名称、类型、枚举值都不变 |
| Phase 字段 | 删除 | 提供迁移文档和工具 |
| ClusterHealthState 字段 | 删除 | 提供迁移文档和工具 |

**行为兼容性**

| 兼容性类型 | 保证措施 | 说明 |
|------------|----------|------|
| 状态转换逻辑 | 保持不变 | 通过映射函数保证逻辑一致 |
| 外部消费者 | 不受影响 | ClusterStatus 保持不变 |
| 内部逻辑 | 简化 | 删除冗余字段，简化代码 |
...
| 性能指标 | 预期提升 | 说明 |
|----------|----------|------|
| 内存使用 | 减少约 5% | 删除两个字段，减少内存占用 |
| CPU 使用 | 减少约 3% | 减少状态同步和转换 |
| 代码可维护性 | 提高约 30% | 删除冗余字段，简化代码 |
| 状态转换延迟 | 减少约 10% | 简化状态转换逻辑 |

**性能监控**

| 监控指标 | 监控方法 | 监控频率 |
|----------|----------|----------|
| 内存使用 | Prometheus + Grafana | 实时监控 |
| CPU 使用 | Prometheus + Grafana | 实时监控 |
| 状态转换延迟 | 日志分析 | 每小时分析 |
| 错误率 | 日志分析 | 实时监控 |
...
| 测试场景 | 测试方法 | 预期结果 |
|----------|----------|----------|
| 集群创建 | 创建 10 个集群 | 平均创建时间 < 10 分钟 |
| 集群升级 | 升级 10 个集群 | 平均升级时间 < 5 分钟 |
| 并发操作 | 同时创建 5 个集群 | 无性能下降 |
