...
### 2.5 状态验证机制（已整合到 2.2 转换表）

**设计决策**：状态验证逻辑已整合到 2.2 节的状态转换表中，通过 `Condition` 字段实现。

**理由**：
1. **避免重复验证**：转换表的 `Condition` 字段已经实现了前置条件检查（如 `isClusterReady`、`isScaleComplete`）
2. **单一数据源**：所有状态转换规则集中在一处定义，避免维护两套验证逻辑
3. **精确匹配**：转换表通过 `FromState + Trigger` 精确匹配，而独立验证器通过 `FromState + ToState` 匹配可能产生歧义
4. **统一失败处理**：所有失败路径统一走 `Error` 触发器，避免两条失败路径（Error 触发器 vs 验证失败）的混淆

**验证函数实现**：原 2.5 节的验证函数已整合到 2.2.2 节的 `conditions.go` 中：
- `isClusterReady`：检查所有节点 Ready + 健康检查通过
- `isScaleComplete`：检查扩缩容完成（节点版本一致 + 数量匹配）
- `isUpgradeComplete`：检查升级完成（所有组件版本达标）
- `isAddonComplete`：检查 Addon 部署完成
- `isManageComplete`：检查纳管完成

详见 2.2.2 节的 `pkg/phaseframe/statemachine/conditions.go` 文件。
...
func TestConditionFunctions(t *testing.T) {
    t.Run("isClusterReady with all nodes ready", func(t *testing.T) {
        cluster := &BKECluster{
            Status: BKEClusterStatus{
                Nodes: []NodeStatus{
                    {IP: "192.168.1.1", State: NodeReady},
                    {IP: "192.168.1.2", State: NodeReady},
                },
            },
        }
        
        cc := &ConditionContext{BKECluster: cluster}
        assert.True(t, isClusterReady(cc))
    })
    ...
    t.Run("isClusterReady with node not ready", func(t *testing.T) {
        cluster := &BKECluster{
            Status: BKEClusterStatus{
                Nodes: []NodeStatus{
                    {IP: "192.168.1.1", State: NodeReady},
                    {IP: "192.168.1.2", State: NodeNotReady},
                },
            },
        }
        
        cc := &ConditionContext{BKECluster: cluster}
        assert.False(t, isClusterReady(cc))
    })
}
```
