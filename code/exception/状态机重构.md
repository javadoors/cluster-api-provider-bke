...
#### 1.1.2 ClusterStatus 与 ClusterHealthState 职责重叠问题

**问题描述**：

`ClusterStatus` 和 `ClusterHealthState` 两个枚举字段存在严重的语义重叠和职责不清问题。
...
**影响**：
- 开发者难以理解应该使用哪个字段
- 状态转换逻辑分散在两个字段中，增加维护成本
- 容易出现状态不一致（ClusterStatus=Upgrading 但 ClusterHealthState=Healthy）

##### 重构方案：重构现有字段（方案 C）

**设计原则**：
- **不新增字段**：保留 `ClusterStatus` 字段，删除 `ClusterHealthState` 字段
- **重新定义语义**：`ClusterStatus` 统一表达集群状态（操作状态 + 健康状态）
- **与 v3 对齐**：枚举值与 v3 `LifecyclePhase` 对齐，为未来迁移做准备
- **成本最优**：避免双写过渡，一次性解决问题
...
| **方案 C** | **重构现有字段（推荐）** | **8-11 天** | **✅ 成本最优** |

**重构后的枚举设计**：

```go
// 保留 ClusterStatus 类型，重新定义枚举值
...
const (
    ...
    // 操作状态
    ClusterInitializing ClusterStatus = "Initializing"  // 对应 v3: Creating
    ClusterUpgrading   ClusterStatus = "Upgrading"      // 对应 v3: Upgrading
    ClusterScalingUp   ClusterStatus = "ScalingUp"      // 对应 v3: Scaling
    ClusterScalingDown ClusterStatus = "ScalingDown"    // 对应 v3: Scaling
    ClusterDeleting    ClusterStatus = "Deleting"       // 对应 v3: Scaling
    ClusterDeploying   ClusterStatus = "Deploying"      // 对应 v3: Creating
    ClusterManaging    ClusterStatus = "Managing"       // 对应 v3: Running
    
    // 失败状态（对应 v3: Failed）
    ClusterInitFailed      ClusterStatus = "InitFailed"
    ...
    ClusterDeployFailed    ClusterStatus = "DeployFailed"
    ...
    ClusterPaused      ClusterStatus = "Paused"  // v3 无对应，特殊处理
)

// 删除 ClusterHealthState 类型
// type ClusterHealthState string  // 已删除
```
...
type BKEClusterStatus struct {
    ...
    // ClusterStatus 统一表达集群状态（操作状态 + 健康状态）
    // 与 v3 LifecyclePhase 对齐
    ClusterStatus ClusterStatus `json:"clusterStatus,omitempty"`
...
stateDiagram-v2
    ...
    InitFailed --> Initializing: 重试(<=10次)
    InitFailed --> Unknown: 重试>10次
    ...
    Ready --> ScalingUp: 开始扩容
    ScalingUp --> Ready: 扩容成功
    ScalingUp --> ScaleFailed: 扩容失败
    ScaleFailed --> ScalingUp: 重试(<=10次)
    ScaleFailed --> Unhealthy: 重试>10次
    
    Ready --> ScalingDown: 开始缩容
    ScalingDown --> Ready: 缩容成功
    ScalingDown --> ScaleFailed: 缩容失败
    
    Ready --> Deploying: 开始部署
    Deploying --> Ready: 部署成功
    Deploying --> DeployFailed: 部署失败
    DeployFailed --> Deploying: 重试(<=10次)
    DeployFailed --> Unhealthy: 重试>10次
...
| 当前状态 | 触发事件 | 目标状态 | 说明 |
|---------|---------|---------|------|
| Ready | 开始初始化 | Initializing | 集群初始化 |
| Initializing | 初始化成功 | Ready | 初始化完成 |
| Initializing | 初始化失败 | InitFailed | 初始化失败 |
| InitFailed | 重试(<=10次) | Initializing | 重试初始化 |
| InitFailed | 重试>10次 | Unknown | 停止重试 |
| Ready | 开始升级 | Upgrading | 集群升级 |
| Upgrading | 升级成功 | Ready | 升级完成 |
| Upgrading | 升级失败 | UpgradeFailed | 升级失败 |
| UpgradeFailed | 重试(<=10次) | Upgrading | 重试升级 |
| UpgradeFailed | 重试>10次 | Unhealthy | 停止重试 |
| Ready | 开始扩容 | ScalingUp | 节点扩容 |
| ScalingUp | 扩容成功 | Ready | 扩容完成 |
| ScalingUp | 扩容失败 | ScaleFailed | 扩容失败 |
| Ready | 开始缩容 | ScalingDown | 节点缩容 |
| ScalingDown | 缩容成功 | Ready | 缩容完成 |
| ScalingDown | 缩容失败 | ScaleFailed | 缩容失败 |
| Ready | 开始部署 | Deploying | Addon部署 |
| Deploying | 部署成功 | Ready | 部署完成 |
| Deploying | 部署失败 | DeployFailed | 部署失败 |
| Ready | 开始纳管 | Managing | 集群纳管 |
| Managing | 纳管成功 | Ready | 纳管完成 |
| Managing | 纳管失败 | ManageFailed | 纳管失败 |
| Ready | 开始删除 | Deleting | 集群删除 |
| Deleting | 删除成功 | [*] | 删除完成 |
| Deleting | 删除失败 | DeleteFailed | 删除失败 |
| Ready | 暂停集群 | Paused | 暂停调谐 |
| Paused | 恢复集群 | Ready | 恢复调谐 |
| Ready | 健康检查失败 | Unhealthy | 集群不健康 |
| Unhealthy | 健康检查成功 | Ready | 集群恢复健康 |

**实施计划**：

**第一阶段：准备（1-2 天）**
1. 在 `bkecluster_consts.go` 中重新定义 `ClusterStatus` 枚举
2. 删除 `ClusterHealthState` 类型定义
3. 添加迁移函数 `MigrateClusterHealthStateToStatus`
4. 更新文档

**第二阶段：迁移 ClusterHealthState（2-3 天）**
1. 修改 22 处 `ClusterHealthState` 代码
2. 使用迁移函数转换到 `ClusterStatus`
3. 测试验证

**第三阶段：重构 ClusterStatus（3-4 天）**
1. 修改 70 处 `ClusterStatus` 代码
2. 更新状态值（与 v3 对齐）
3. 测试验证

**第四阶段：清理（1-2 天）**
1. 删除 `ClusterHealthState` 字段
2. 删除迁移函数
3. 更新测试用例
4. 更新文档
...
```go
// MigrateClusterHealthStateToStatus 将旧的 ClusterHealthState 转换为 ClusterStatus
// 临时使用，迁移完成后删除
func MigrateClusterHealthStateToStatus(healthState ClusterHealthState) ClusterStatus {
    switch healthState {
    case Deploying:
        return ClusterDeploying
    case DeployFailed:
        return ClusterDeployFailed
    case Upgrading:
        return ClusterUpgrading
    case UpgradeFailed:
        return ClusterUpgradeFailed
    case Managing:
        return ClusterManaging
    case ManageFailed:
        return ClusterManageFailed
    case Unhealthy:
        return ClusterUnhealthy
    case Healthy:
        return ClusterReady
    case Deleting:
        return ClusterDeleting
    default:
...
func (s ClusterStatus) MapToV3LifecyclePhase() LifecyclePhase {
    ...
    case ClusterInitializing, ClusterDeploying:
        return LifecyclePhaseCreating
    case ClusterUpgrading:
        return LifecyclePhaseUpgrading
    case ClusterScalingUp, ClusterScalingDown, ClusterDeleting:
        return LifecyclePhaseScaling
    case ClusterInitFailed, ClusterUpgradeFailed, ClusterScaleFailed,
         ClusterDeleteFailed, ClusterDeployFailed, ClusterManageFailed,
...
| 当前方案状态 | v3 LifecyclePhase | 映射说明 |
|------------|------------------|---------|
| `Ready` | `Running` | 就绪状态对应运行状态 |
| `Initializing` | `Creating` | 初始化阶段对应创建阶段 |
| `Upgrading` | `Upgrading` | 升级状态直接对应 |
| `ScalingUp` | `Scaling` | 扩容状态对应扩缩容状态 |
| `ScalingDown` | `Scaling` | 缩容状态对应扩缩容状态 |
| `Deleting` | `Scaling` | 删除状态对应缩容状态 |
| `Deploying` | `Creating` | 部署阶段对应创建阶段 |
| `Managing` | `Running` | 纳管状态对应运行状态 |
| `InitFailed` | `Failed` | 初始化失败对应失败状态 |
| `UpgradeFailed` | `Failed` | 升级失败对应失败状态 |
| `ScaleFailed` | `Failed` | 扩缩容失败对应失败状态 |
| `DeleteFailed` | `Failed` | 删除失败对应失败状态 |
| `DeployFailed` | `Failed` | 部署失败对应失败状态 |
| `ManageFailed` | `Failed` | 纳管失败对应失败状态 |
| `Unhealthy` | `Failed` | 不健康状态对应失败状态 |
| `Unknown` | `Failed` | 未知状态对应失败状态 |
| `Paused` | `Running` | v3 无暂停状态，映射到运行 |

**需要更新的文件清单**：

| 文件 | 修改类型 | 预计修改量 |
|------|---------|-----------|
| `api/capbke/v1beta1/bkecluster_consts.go` | 重构 ClusterStatus 枚举，删除 ClusterHealthState | 约 30 行 |
| `api/bkecommon/v1beta1/bkecluster_status.go` | 删除 ClusterHealthState 字段 | 约 5 行 |
| `pkg/phaseframe/phases/phase_flow.go` | 修改状态转换逻辑（22 处） | 约 20 行 |
| `pkg/statusmanage/statusmanager.go` | 修改失败处理逻辑（5 处） | 约 10 行 |
| `pkg/phaseframe/phases/ensure_*.go` | 修改各 Phase 状态设置（43 处） | 约 40 行 |
| `controllers/capbke/bkecluster_controller.go` | 修改控制器逻辑（2 处） | 约 5 行 |
...
| 外部消费者影响 | 低 | 中 | ClusterStatus 是内部字段，影响有限 |
| 枚举值设计不合理 | 低 | 中 | 参考现有代码，充分评审 |

**与 v3 设计的关系**：

- **当前方案定位**：针对 PhaseFlow 的改进，解决 ClusterStatus 和 ClusterHealthState 职责重叠问题
- **v3 方案定位**：未来的目标架构，三层状态机设计
- **对齐关系**：当前方案的状态通过 `MapToV3LifecyclePhase` 函数映射到 v3 状态
