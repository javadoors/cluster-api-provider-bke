...
##### 重构方案：合并为单一 ClusterState 字段

**设计原则**：
- 合并为一个 `ClusterState` 字段
- 不使用 Condition 机制（BKE 规模不需要）
- 枚举值表达完整的集群状态（操作状态 + 健康状态）
...
const (
    ...
    ClusterStateScalingUp   ClusterState = "ScalingUp"
    ...
    ClusterStateManaging    ClusterState = "Managing"
    
    // 失败状态
    ClusterStateInitFailed      ClusterState = "InitFailed"
    ClusterStateUpgradeFailed   ClusterState = "UpgradeFailed"
    ClusterStateScaleFailed     ClusterState = "ScaleFailed"
    ClusterStateDeleteFailed    ClusterState = "DeleteFailed"
    ClusterStateDeployFailed    ClusterState = "DeployFailed"
    ClusterStateManageFailed    ClusterState = "ManageFailed"
...
**第一阶段：定义新字段（1-2 天）**
1. 在 `bkecluster_consts.go` 中定义 `ClusterState` 枚举
2. 在 `BKEClusterStatus` 中添加 `ClusterState` 字段
3. 标记旧字段为 `@deprecated`

**第二阶段：并行运行（3-5 天）**
1. 在新代码中使用 `ClusterState`
2. 保持旧字段同步更新
3. 添加转换函数：`ClusterStateFromLegacy(status, health)`

**第三阶段：逐步迁移（5-7 天）**
1. 更新 `phase_flow.go` 中的状态转换逻辑
2. 更新 `statusmanager.go` 中的失败处理逻辑
3. 更新所有 `ensure_*.go` 文件
4. 更新控制器代码
...
| 文件 | 修改类型 | 预计修改量 |
|------|---------|-----------|
| `api/capbke/v1beta1/bkecluster_consts.go` | 新增 ClusterState 枚举 | 约 30 行 |
| `api/bkecommon/v1beta1/bkecluster_status.go` | 新增 ClusterState 字段 | 约 5 行 |
| `pkg/phaseframe/phases/phase_flow.go` | 修改状态转换逻辑 | 约 20 行 |
| `pkg/statusmanage/statusmanager.go` | 修改失败处理逻辑 | 约 15 行 |
| `pkg/phaseframe/phases/ensure_*.go` | 修改各 Phase 状态设置 | 约 50 行 |
| `controllers/capbke/bkecluster_controller.go` | 修改控制器逻辑 | 约 10 行 |
| `*_test.go` | 更新测试用例 | 约 100 行 |
| `code/specification/升级规格.md` | 更新规格文档 | 约 50 行 |
