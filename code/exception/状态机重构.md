...
```go
// 状态转换规则定义（与 engine.go 中的 Transition 结构体一致）
type Transition struct {
    FromState ClusterStatus
    ToState   ClusterStatus
    Trigger   string
    Condition func(*BKECluster) bool  // 转换前置条件（可选）
    Action    func(*BKECluster) error // 转换动作（可选）
}

// 状态转换表（设计文档，实际代码实现见 2.2.3 节的 registerClusterTransitions 函数）
// 以下声明式表用于设计评审和文档参考，编程式注册用于运行时初始化
var ClusterStateTransitionTable = []Transition{
    // ========== 初始化阶段 ==========
...
```go
// ❌ 错误实现：err 参数从未使用
func (e *Engine) Transition(cluster *BKECluster, trigger string, err error) error {
...
// 本函数是上方声明式转换表 ClusterStateTransitionTable 的代码实现，两者一一对应
...
type Transition struct {
    ...
    Trigger   string
    Condition func(*bkev1beta1.BKECluster) bool  // 转换前置条件（可选）
