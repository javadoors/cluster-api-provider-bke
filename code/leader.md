# 一页PPT排版方案：BKE 升级架构重构规划

## 页面布局设计
```
┌─────────────────────────────────────────────────────────────────────────────────────┐
│  标题：BKE 集群升级架构重构 —— 从"命令式黑盒"到"声明式可观测"                          │
├──────────────────────────┬──────────────────────────┬───────────────────────────────┤
│  📌 现状痛点（8大问题）   │  🚀 新架构能力与收益      │  📅 重构交付节奏（6个月）      │
│                          │                          │                               │
│  1. 版本管理缺失          │  ✅ ClusterVersion CRD   │  M1: 基础层 (CRD/OCI/路径图)  │
│     4个字段分散，无整体   │     统一版本概念          │     Alpha 发布                │
│     版本概念              │     排查: 2h → 5min      │                               │
│                          │                          │  M2-3: 控制层 (3大控制器)     │
│  2. 不支持升级路径        │  ✅ UpgradePath 图算法   │     Beta 发布                 │
│     直接跨版本，无中间    │     多跳升级，每跳可验证  │                               │
│     过渡                  │     中断: 4h → 30min     │  M4: 调度层 (DAG/Phase适配)  │
│                          │                          │     RC 发布                   │
│  3. 无兼容性预检          │  ✅ CSP 约束求解器       │                               │
│     升级前不校验兼容性    │     升级前拦截不兼容组合  │  M5: 交付层 (迁移/E2E/压测)   │
│     失败率 30%+           │     失败率: 30% → <5%    │     测试通过率 >95%           │
│                          │                          │                               │
│  4. 升级过程黑盒          │  ✅ 全链路状态追踪       │  M6: 全量切换 (GA 发布)       │
│     只有成功/失败状态     │     实时进度+WebSocket   │     升级成功率 >95%           │
│     定位耗时 1h+          │     定位: 1h → 3min      │                               │
│                          │                          │                               │
│  5. 组件升级强耦合        │  ✅ ReleaseImage +       │  ┌─────────────────────────┐  │
│     所有组件绑定升级      │     bke-manifests 外置   │  │ 资源投入: 102 人天       │  │
│     新增组件需改核心代码  │     新组件上线: 2月→1周  │  │ 峰值人力: 3 人并行       │  │
│                          │                          │  └─────────────────────────┘  │
│  6. 失败回滚困难          │  ✅ FailurePolicy 策略   │                               │
│     人工回滚，易出错      │     自动回滚+检查点      │  ┌─────────────────────────┐  │
│     Etcd 数据可能丢失     │     数据丢失风险: → 0    │  │ 核心收益                 │  │
│                          │                          │  │ 升级成功率: 70% → >95%  │  │
│  7. 离线环境不支持        │  ✅ ManifestStore 多级   │  │ 平均升级时间: 缩短 60%  │  │
│     断网无法升级          │     缓存，支持离线降级    │  │ 故障恢复: 缩短 90%      │  │
│     政企客户无法使用      │     实施成本: ↓90%       │  │ 新组件上线: 缩短 85%    │  │
│                          │                          │  └────────────────────────-┘  │
│  8. 安装升级代码混用      │  ✅ 安装/升级 DAG 独立   │                               │
│     26个Phase条件判断     │     代码分支 ↓70%        │                               │
│     新人上手 3 个月       │     上手: 3月 → 2 周     │                               │
├──────────────────────────┴──────────────────────────┴───────────────────────────────┤
│  💡 总结：当前架构是"命令式、黑盒、强耦合"，已成为商业化落地瓶颈。新架构 6 个月交付，    │
│     升级成功率提升至 95%+，故障恢复时间缩短 90%，全面打开政企与边缘市场。               │
└─────────────────────────────────────────────────────────────────────────────────────┘
```
...
| 区域 | 配色建议 | 图标 | 说明 |
|------|---------|------|------|
| **现状痛点** | 🔴 红色背景或红色边框 | ⚠️ 警告图标 | 突出问题严重性，用数字强化冲击 |
| **新架构能力** | 🟢 绿色背景或绿色边框 | ✅ 对勾图标 | 展示解决方案，与痛点一一对应 |
| **交付节奏** | 🔵 蓝色背景或蓝色边框 | 📅 日历图标 | 体现计划性和可控性 |
...
### 3. 给领导的讲解要点（3 分钟话术）
**第 1 分钟：痛点引入**
> "领导，当前 BKE 升级架构存在 8 个核心问题。最致命的是三点：第一，没有统一版本概念，出了事查问题要 2 小时；第二，不支持升级路径，跨版本升级失败率 30%+，业务中断 4 小时起；第三，组件升级强耦合，客户想修一个安全漏洞，要等 1-2 个月发版。这已经成为我们政企客户落地的最大障碍。"

**第 2 分钟：方案亮点**
> "KEP-5 新架构引入三大核心能力：ClusterVersion 统一版本管理、UpgradePath 图算法实现多跳升级、CSP 求解器升级前预检。升级成功率从 70% 提升到 95%+，故障恢复从 4 小时缩短到 15 分钟，新组件上线从 2 个月缩短到 1 周。"

**第 3 分钟：交付承诺**
> "整个重构 6 个月完成，总投入 102 人天，峰值 3 人并行。M1 出 Alpha，M3 出 Beta，M6 GA 发布全量切换。完成后，政企和边缘市场的离线升级需求将全面支持，预计客户投诉率下降 80%。"

## PPT 制作实操建议
1. **字体**：标题用 24-28pt，正文用 14-16pt，数据用 18pt 加粗
2. **行距**：1.2 倍行距，避免拥挤
3. **对齐**：三栏严格等宽，左对齐
4. **留白**：每栏之间留 1-2cm 间距，底部总结栏占全高 15%
5. **动画**（可选）：痛点→方案→节奏 依次出现，避免信息过载
...
| **9** | **非镜像组件无法升级** | 升级方案仅覆盖容器镜像组件，对于非镜像形态的组件（如节点上的二进制文件 kubelet/containerd、systemd 服务配置、内核参数、证书文件等）无法通过修改镜像 tag 来升级 | **集群升级不完整**，核心组件如 kubelet、containerd 仍停留在旧版本，存在安全漏洞和兼容性隐患，**整体升级效果打折扣** |

> **当前基于修改镜像 Tag 的升级方案是"碎片化、黑盒、强耦合"的，仅能覆盖容器镜像组件的更新，缺乏整体版本管理、兼容性预检、升级路径规划和自动回滚能力。对于非镜像组件（二进制、配置、证书等）完全无能为力，升级失败率高、故障恢复慢，已成为制约产品商业化落地的核心瓶颈。**
## 总结
...
## 针对现有架构问题的解决方案
| 序号 | 现有问题 | 新架构提供的能力 | 带来的收益 |
|:---:|---------|-----------------|-----------|
| **1** | **版本管理缺失**<br>版本信息散落在 4 个独立字段中 | **ClusterVersion CRD**：引入统一的集群版本概念，1:1 关联 BKECluster，通过 `spec.desiredVersion` 和 `status.currentVersion` 集中管理版本 | **领导可一句话回答"集群当前是 v2.5.0"**；版本变更有完整审计日志，故障排查时间从 **2 小时缩短至 5 分钟** |
| **2** | **不支持升级路径**<br>直接跨版本升级，无中间过渡 | **UpgradePath CRD + 图算法**：定义版本间合法流转关系，BFS 自动计算最短路径（如 v2.4→v2.5→v2.6），支持多跳逐跳执行，每跳完成后可暂停验证 | **跨大版本升级风险降低 90%**；跳过中间版本导致的数据损坏问题彻底消除；升级失败可回退到上一跳稳定版本，**业务中断时间从 4 小时缩短至 30 分钟** |
| **3** | **无兼容性预检**<br>升级前不检查组件兼容性 | **CSP 约束求解器**：升级前自动校验组件版本兼容性（如 K8s v1.29 要求 Etcd ≥v3.5.10），不兼容直接拦截并输出详细冲突报告 | **升级失败率从 30%+ 降至 5% 以下**；客户投诉率大幅下降；预检报告可作为升级审批依据，满足合规要求 |
| **4** | **升级过程黑盒**<br>只有成功/失败两个状态 | **全链路 Phase 状态追踪**：`ClusterVersion.Status` 实时记录当前执行跳数、当前步骤、已完成步骤列表、预计剩余时间；WebSocket 推送实时日志 | **运维可随时向业务方同步精确进度**；故障定位时间从 **1 小时缩短至 3 分钟**；支持生成升级报告用于审计 |
| **5** | **组件升级耦合**<br>所有组件绑定升级，新增组件需改核心代码 | **ComponentFactory + ComponentVersion 注册机制**：组件通过 YAML 清单或注册到 Factory 独立声明，DAG 按依赖关系调度；新增组件只需添加 ComponentVersion 定义，**无需修改核心调度代码** | **组件可独立升级**（如只升级监控组件）；新增组件上线周期从 **1-2 个月缩短至 1 周**；核心代码零侵入，发版风险大幅降低 |
| **5** | **组件升级强耦合** | 所有组件的镜像 tag 在一次操作中统一修改，无法单独升级某个组件（如只升级监控组件而不升级 K8s）；新增一个组件的镜像升级需要修改核心调度代码并重新发版 | **ReleaseImage 声明式组件清单**：每个版本的组件列表声明在 `ReleaseImage` CR 中，包含安装组件和升级组件两个独立列表，按需声明、按需升级。<br><br>**组件外置化（bke-manifests）**：所有组件定义外置到 `bke-manifests` 目录，以 `ComponentVersion` YAML 声明，支持 YAML/Helm/Binary/Inline 四种类型。新增组件只需在 `bke-manifests` 中添加组件目录和 `component.yaml`，**无需修改任何核心调度代码**。<br><br>**ComponentFactory 注册机制**：代码实现的 Phase 统一注册到 `ComponentFactory`，通过 `(name, version)` 索引，ReleaseImage 中直接引用即可。 | **组件可独立升级**：只升级声明在 `ReleaseImage.Spec.Upgrade.Components` 中的组件，未声明的组件不受影响。<br><br>**新增组件上线周期从 1-2 个月缩短至 1 周**：只需添加 `bke-manifests` 目录和 CR 引用，核心代码零侵入。<br><br>**发版风险降低 90%**：组件独立演进，单个组件问题不影响其他组件；支持 A/B 测试和灰度发布。 |
| **6** | **失败回滚困难**<br>无自动回滚，人工恢复易出错 | **FailurePolicy 策略**：支持 `FailFast`（立即终止）、`Continue`（跳过非核心组件）、`Rollback`（自动降级到上一稳定版本）；多跳升级每跳创建检查点 | **核心组件升级失败自动回滚**，无需人工介入；Etcd 等关键组件升级前自动备份，**数据丢失风险降至 0** |
| **7** | **离线环境不支持**<br>依赖在线拉取资源 | **ManifestStore + OCI 多级缓存**：优先从本地 `/etc/bke/manifests` 加载，失败后降级至 OCI 拉取；支持 OCI Layout 本地 tarball 导入 | **政企/边缘客户可在断网环境下升级**；离线升级实施成本降低 **90%**；新市场机会打开 |
| **8** | **安装升级代码混用**<br>26 个 Phase 通过复杂条件判断区分场景 | **安装 DAG 与升级 DAG 独立构建**：根据 `ClusterVersion` 场景自动选择安装或升级流程；Phase 复用 `NeedExecute()` 接口，通过版本上下文自动判断是否执行 | **代码分支减少 70%**；新人上手时间从 **3 个月缩短至 2 周**；测试工作量减半，历史 Bug 重复率降低 **80%** |

## 总体收益对比
| 指标 | 当前架构 | KEP-5 新架构 | 提升幅度 |
|------|---------|-------------|---------|
| **升级成功率** | ~70% | >95% | **+25%** |
| **平均升级时间** | 2-4 小时 | 30-60 分钟 | **缩短 60%** |
| **故障恢复时间** | 4+ 小时（人工） | 15 分钟（自动回滚） | **缩短 90%** |
| **新组件上线周期** | 1-2 个月 | 1 周 | **缩短 85%** |
| **离线环境支持** | 不支持 | 完整支持 | **从 0 到 1** |
| **客户投诉率** | 高 | 低 | **下降 80%+** |

# 基于修改镜像 Tag 的升级方案

## 📌 问题描述
> 该升级方案仅依赖镜像 Tag 的修改来触发更新，缺乏对组件级别的声明式管理。升级逻辑无法覆盖 **组件 Pod 的配置变化**（如资源限制、环境变量、挂载点）以及 **新增 CRD** 等声明式对象的引入。由于没有与组件级别的抽象对应，升级过程停留在镜像层面，无法完整表达组件的生命周期与依赖关系。

## 💥 影响分析
- **缺乏组件抽象**  
  升级仅限于镜像版本，无法在概念上对应到组件级别，导致配置和 CRD 变更无法统一管理。

- **升级不完整**  
  Pod 配置变化、新增 CRD 等无法通过镜像 Tag 修改实现，升级过程碎片化，容易出现部分更新遗漏。

- **一致性风险**  
  镜像版本更新与组件配置更新分离，可能导致运行环境与镜像版本不匹配，增加故障风险。

- **回滚困难**  
  单纯回退镜像 Tag 并不能恢复配置或 CRD 的变更，缺乏完整的回滚能力。

# KEP-5 架构重构节奏规划

## 总体重构路线图（6 个月）
```
Phase 1 (M1)        Phase 2 (M2-M3)       Phase 3 (M4)        Phase 4 (M5)        Phase 5 (M6)
┌─────────────┐    ┌─────────────────┐    ┌─────────────┐    ┌─────────────┐    ┌─────────────┐
│  基础层建设   │    │   控制层开发     │    │  调度层集成   │    │  交付层完善   │    │  全量切换    │
│             │    │                 │    │             │    │             │    │             │
│ CRD 定义    │───▶│ 控制器开发       │───▶│ DAG 引擎    │───▶│ 平滑迁移    │───▶│ GA 发布     │
│ OCI 解析层  │    │ 预检引擎        │    │ Phase 适配  │    │ E2E 验证    │    │ 移除旧逻辑   │
│ 升级路径图  │    │ 兼容性校验      │    │ 安装器框架  │    │ 压测优化    │    │             │
└─────────────┘    └─────────────────┘    └─────────────┘    └─────────────┘    └─────────────┘
     102 人天总计
```
...
### Phase 1：基础层建设（第 1 个月）
| 维度 | 内容 |
|------|------|
| **目标** | 完成版本管理基础设施搭建，定义核心数据模型 |
| **核心任务** | ① 定义 ClusterVersion、ReleaseImage、UpgradePath、ComponentVersion 四大 CRD<br>② 实现 OCI 镜像解析层，支持从仓库拉取版本清单<br>③ 构建 UpgradePath 图模型，实现路径查找算法 |
| **交付物** | CRD YAML 定义、API Go 代码、Webhook 校验规则、OCI 客户端、路径图算法库 |
| **里程碑** | ✅ CRD 注册成功，`kubectl get clusterversion/releaseimage/upgradepath` 可正常查询 |
| **风险** | kubebuilder 脚手架学习曲线较陡，CRD 验证规则调试耗时 |

### Phase 2：控制层开发（第 2-3 个月）
| 维度 | 内容 |
|------|------|
| **目标** | 实现三大控制器，完成版本预检与兼容性校验能力 |
| **核心任务** | ① ClusterVersionReconciler：监听版本变更，触发升级流程<br>② ReleaseImageReconciler：验证 OCI 签名，解析组件清单，执行 CSP 兼容性求解<br>③ UpgradePathReconciler：维护全局升级路径图，监控 latest digest 变更<br>④ 预检引擎集成：路径查找 + 兼容性校验 |
| **交付物** | 三个控制器代码、CSP 求解器、兼容性校验报告、Digest 监控模块 |
| **里程碑** | ✅ 修改 ClusterVersion.DesiredVersion 后，自动触发预检并输出升级路径报告 |
| **风险** | controller-runtime 调谐循环幂等性设计复杂，Watch 过滤需精细调优 |

### Phase 3：调度层集成（第 4 个月）
| 维度 | 内容 |
|------|------|
| **目标** | 实现 DAG 调度引擎，完成 Phase 适配与统一安装器框架 |
| **核心任务** | ① DAG 引擎：依赖图构建、拓扑排序、FailurePolicy 分支、并发调度<br>② Phase 适配：4 个核心 Phase 转为 YAML 清单，其余注册到 ComponentFactory<br>③ 统一安装器：支持 YAML/Helm/Binary/Inline 四种组件类型的统一 Install/Uninstall/HealthCheck 接口<br>④ VersionContext 注入：复用 NeedExecute() 实现版本比对决策 |
| **交付物** | DAG 调度器代码、ComponentFactory 注册表、四种安装器实现、Phase YAML 清单 |
| **里程碑** ✅ 执行升级 DAG，各类型组件按依赖拓扑顺序完成升级 |
| **风险** | 图算法并发安全与超时控制易出 Bug，上下文注入调试复杂 |

### Phase 4：交付层完善（第 5 个月）
| 维度 | 内容 |
|------|------|
| **目标** | 实现平滑迁移方案，完成全场景 E2E 验证与压测 |
| **核心任务** | ① Feature Gate 灰度机制：新旧架构双轨运行<br>② 自动迁移：为存量集群自动创建 ClusterVersion 实例<br>③ E2E 测试：补丁升级、跨版本升级、多跳升级、失败回滚、OCI 降级<br>④ 性能压测：万级节点并发升级、DAG 构建耗时 <2s、内存泄漏检测 |
| **交付物** | 迁移脚本、E2E 测试套件、压测报告、用户文档 |
| **里程碑** | ✅ E2E 测试通过率 >95%，旧集群可平滑迁移至新架构 |
| **风险** | 真实集群环境搭建耗时，异常场景模拟复杂 |

### Phase 5：全量切换（第 6 个月）
| 维度 | 内容 |
|------|------|
| **目标** | GA 发布，移除旧版本硬编码逻辑，全量切换至新架构 |
| **核心任务** | ① 移除旧 PhaseFrame 硬编码调度逻辑<br>② 清理废弃代码和兼容性开关<br>③ 生产环境灰度发布<br>④ 编写运维手册和故障排查指南 |
| **交付物** | GA 版本发布、运维手册、故障排查指南、架构文档 |
| **里程碑** | ✅ 生产环境全量切换，旧代码完全移除，升级成功率 >95% |
| **风险** | 生产环境灰度需密切监控，回滚预案必须就绪 |

## 资源投入评估
| 阶段 | 工作量 | 人力配置建议 | 关键依赖 |
|------|--------|-------------|---------|
| Phase 1 | 23 人天 | 2 人并行（CRD + OCI） | 无 |
| Phase 2 | 36 人天 | 3 人并行（3 个控制器） | Phase 1 完成 |
| Phase 3 | 36 人天 | 3 人并行（DAG + Phase + 安装器） | Phase 1-2 完成 |
| Phase 4 | 27 人天 | 2 人（迁移 + E2E） | Phase 1-3 完成 |
| Phase 5 | 10 人天 | 1 人（清理 + 发布） | Phase 1-4 完成 |
| **总计** | **102 人天** | **峰值 3 人并行** | — |
...
│           │           │           │           │           │
▼           ▼           ▼           ▼           ▼           ▼
Alpha 发布 ──▶ Beta 发布 ──▶ RC 发布 ──▶ E2E 通过 ──▶ GA 发布
(CRD 可用)   (控制器可用)  (DAG 可用)  (迁移可用)   (全量切换)
```
- **影响扩展性**  
  新增组件或扩展功能时，需要额外修改配置或引入 CRD，镜像 Tag 升级无法覆盖这些场景，限制了架构的灵活性。

## ✅ 总结
基于修改镜像 Tag 的升级方案在 **镜像版本更新** 场景下简单有效，但由于缺乏 **组件级别的抽象与声明式管理**，在涉及 **配置变化** 和 **新增 CRD** 的复杂升级中存在明显局限性。这会导致升级不完整、一致性风险增加、回滚困难，并限制系统的扩展性。


# KEP-5 架构重构节奏规划

## 总体重构路线图
| 阶段 | 周期 | 阶段名称 | 核心任务 | 交付物 | 里程碑 | 风险与缓解 |
|:---:|:---:|---------|---------|--------|--------|-----------|
| **Phase 1** | 第 1 个月 | **基础层建设** | ① 定义四大核心 CRD（ClusterVersion/ReleaseImage/UpgradePath/ComponentVersion）<br>② 实现 OCI 镜像解析层，支持版本清单拉取<br>③ 构建 UpgradePath 图模型与路径查找算法 | CRD YAML 定义、API Go 代码、Webhook 校验规则、OCI 客户端、路径图算法库 | ✅ CRD 注册成功，`kubectl get clusterversion/releaseimage/upgradepath` 可正常查询 | **风险**：kubebuilder 学习曲线陡<br>**缓解**：安排有经验的开发者带头 |
| **Phase 2** | 第 2-3 个月 | **控制层开发** | ① ClusterVersionReconciler：监听版本变更，触发升级<br>② ReleaseImageReconciler：验证 OCI 签名，执行 CSP 兼容性求解<br>③ UpgradePathReconciler：维护全局路径图，监控 digest 变更<br>④ 预检引擎集成 | 三个控制器代码、CSP 求解器、兼容性校验报告、Digest 监控模块 | ✅ 修改 DesiredVersion 后，自动触发预检并输出升级路径报告 | **风险**：调谐循环幂等性设计复杂<br>**缓解**：充分单元测试覆盖 |
| **Phase 3** | 第 4 个月 | **调度层集成** | ① DAG 引擎：依赖图构建、拓扑排序、并发调度<br>② Phase 适配：4 个 Phase 转 YAML，其余注册到 ComponentFactory<br>③ 统一安装器：支持 YAML/Helm/Binary/Inline 四种类型<br>④ VersionContext 注入，复用 NeedExecute() | DAG 调度器、ComponentFactory 注册表、四种安装器、Phase YAML 清单 | ✅ 执行升级 DAG，各组件按依赖拓扑顺序完成升级 | **风险**：图算法并发安全易出 Bug<br>**缓解**：引入 errgroup 控制并发 |
| **Phase 4** | 第 5 个月 | **交付层完善** | ① Feature Gate 灰度机制，新旧双轨运行<br>② 自动迁移：为存量集群创建 ClusterVersion<br>③ E2E 测试：补丁/跨版本/多跳升级、失败回滚、断网降级<br>④ 性能压测：万级节点并发、DAG 构建 <2s | 迁移脚本、E2E 测试套件、压测报告、用户文档 | ✅ E2E 通过率 >95%，旧集群可平滑迁移 | **风险**：真实环境搭建耗时<br>**缓解**：提前准备 Kind 测试集群 |
| **Phase 5** | 第 6 个月 | **全量切换** | ① 移除旧 PhaseFrame 硬编码逻辑<br>② 清理废弃代码和兼容性开关<br>③ 生产环境灰度发布<br>④ 编写运维手册和故障排查指南 | GA 版本发布、运维手册、故障排查指南、架构文档 | ✅ 生产全量切换，升级成功率 >95% | **风险**：生产灰度需密切监控<br>**缓解**：回滚预案必须就绪 |
...
| 时间节点 | 里程碑 | 就绪标准 |
|:---:|:---:|---------|
| **M1 末** | Alpha 发布 | CRD 可用，版本清单可解析 |
| **M3 末** | Beta 发布 | 控制器可用，预检引擎可输出报告 |
| **M4 末** | RC 发布 | DAG 可用，组件按拓扑顺序升级 |
| **M5 末** | E2E 通过 | 迁移可用，测试通过率 >95% |
| **M6 末** | GA 发布 | 全量切换，旧代码完全移除 |
