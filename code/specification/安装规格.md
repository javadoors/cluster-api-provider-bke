...
> 本文档梳理 cluster-api-provider-bke 工程代码实现的安装相关的完整规格，覆盖安装场景、状态机、失败处理与重试机制四个维度。
...
- [3. 安装失败处理规格](#3-安装失败处理规格)
  ...
  - [3.2 各 Phase 失败处理详情](#32-各-phase-失败处理详情)
  - [3.3 失败处理汇总](#33-失败处理汇总)
- [4. 安装重试规格](#4-安装重试规格)
  - [4.1 三层重试机制总览](#41-三层重试机制总览)
  - [4.2 各 Phase 重试配置](#42-各-phase-重试配置)
...
Phase 注册见 [list.go:35-69](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/list.go#L35-L69)。

### 1.2 Phase 入口与执行机制

#### 1.2.1 Phase 注册入口

**文件位置**：[pkg/phaseframe/phases/list.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/list.go)
...
**文件位置**：[pkg/phaseframe/phases/phase_flow.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/phase_flow.go)
...
**位置**：[ensure_finalizer.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_finalizer.go)
...
**位置**：[ensure_paused.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_paused.go)
...
**位置**：[ensure_cluster_manage.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_cluster_manage.go)
...
**位置**：[ensure_delete_or_reset.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_delete_or_reset.go)
...
**位置**：[ensure_dry_run.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_dry_run.go)
...
**位置**：[ensure_bke_agent.go:63-96](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_bke_agent.go#L63-L96)
...
**位置**：[ensure_nodes_env.go:351-388](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_nodes_env.go#L351-L388)
...
**位置**：[ensure_cluster_api_obj.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_cluster_api_obj.go)
...
**位置**：[ensure_certs.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_certs.go)
...
**位置**：[ensure_load_balance.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_load_balance.go)
...
**位置**：[ensure_master_init.go:473-531](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_master_init.go#L473-L531)
...
**位置**：[ensure_master_join.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_master_join.go)
...
**位置**：[ensure_worker_join.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_worker_join.go)
...
**位置**：[ensure_addon_deploy.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_addon_deploy.go)
...
**位置**：[ensure_nodes_postprocess.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_nodes_postprocess.go)
...
**位置**：[ensure_agent_switch.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_agent_switch.go)
...
**位置**：[api/capbke/v1beta1/bkecluster_consts.go:151-183](https://gitcode.com/openfuyao/cluster-api-provider-bke/api/capbke/v1beta1/bkecluster_consts.go#L151-L183)
...
| `ClusterDeployingAddon` | DeployingAddon | Addon 部署中间态 |
| `ClusterDeployAddonFailed` | DeployAddonFailed | Addon 部署失败态 |
| `ClusterManaging` | Managing | 纳管中间态 |
| `ClusterManageFailed` | ManageFailed | 纳管失败态 |
| `ClusterPaused` | Paused | 暂停中间态 |
| `ClusterPauseFailed` | PauseFailed | 暂停失败态 |
| `ClusterDryRun` | DryRun | DryRun 中间态 |
| `ClusterDryRunFailed` | DryRunFailed | DryRun 失败态 |
| `ClusterDeleting` | Deleting | 删除中间态 |
| `ClusterDeleteFailed` | DeleteFailed | 删除失败态 |

#### 2.1.2 集群健康状态（ClusterHealthState）

**位置**：[api/capbke/v1beta1/bkecluster_consts.go:222-230](https://gitcode.com/openfuyao/cluster-api-provider-bke/api/capbke/v1beta1/bkecluster_consts.go#L222-L230)
...
**位置**：[api/bkecommon/v1beta1/bkenode_types.go:36-42](https://gitcode.com/openfuyao/cluster-api-provider-bke/api/bkecommon/v1beta1/bkenode_types.go#L36-L42) + [api/capbke/v1beta1/bkecluster_consts.go:197-218](https://gitcode.com/openfuyao/cluster-api-provider-bke/api/capbke/v1beta1/bkecluster_consts.go#L197-L218)
...
**位置**：[api/capbke/v1beta1/bkecluster_consts.go:233-246](https://gitcode.com/openfuyao/cluster-api-provider-bke/api/capbke/v1beta1/bkecluster_consts.go#L233-L246)
...
stateDiagram-v2
    ...
    state "升级场景" as UpgradeScenario {
        ClusterReady --> ClusterUpgrading: spec 版本变更
        ClusterUpgrading --> ClusterReady: 升级成功
        ClusterUpgrading --> ClusterUpgradeFailed: 升级失败
    }
    
    state "扩容场景" as ScaleUpScenario {
    ...
    state "缩容场景" as ScaleDownScenario {
        ...
        ClusterMasterScalingDown --> ClusterScaleFailed: 缩容失败
        
        ClusterWorkerScalingDown --> ClusterReady: 缩容成功
        ClusterWorkerScalingDown --> ClusterScaleFailed: 缩容失败
    }
    
    state "Addon 部署场景" as AddonScenario {
        ClusterReady --> ClusterDeployingAddon: 部署 Addon
        ClusterDeployingAddon --> ClusterReady: 部署成功
        ClusterDeployingAddon --> ClusterDeployAddonFailed: 部署失败
    }
    
    state "纳管场景" as ManageScenario {
        ClusterReady --> ClusterManaging: 纳管集群
        ClusterManaging --> ClusterReady: 纳管成功
        ClusterManaging --> ClusterManageFailed: 纳管失败
    }
    
    state "暂停场景" as PauseScenario {
        ClusterReady --> ClusterPaused: 暂停管理
        ClusterPaused --> ClusterReady: 恢复管理
        ClusterPaused --> ClusterPauseFailed: 暂停失败
    }
    
    state "DryRun 场景" as DryRunScenario {
        ClusterReady --> ClusterDryRun: DryRun 模式
        ClusterDryRun --> ClusterReady: DryRun 完成
        ClusterDryRun --> ClusterDryRunFailed: DryRun 失败
    }
    
    state "删除场景" as DeleteScenario {
        ClusterReady --> ClusterDeleting: 删除集群
        ClusterDeleting --> [*]: 删除成功
        ClusterDeleting --> ClusterDeleteFailed: 删除失败
    }
    
    state "健康检查场景" as HealthScenario {
        ClusterReady --> ClusterChecking: 健康检查中
        ClusterChecking --> ClusterReady: 检查通过
        ClusterChecking --> ClusterUnhealthy: 检查失败
        ClusterUnhealthy --> ClusterReady: 恢复健康
    }
    
    ClusterScaleFailed --> ClusterReady: 重试成功
    ClusterUpgradeFailed --> ClusterReady: 重试成功
    ClusterManageFailed --> ClusterReady: 重试成功
    ClusterDeployAddonFailed --> ClusterReady: 重试成功
    ClusterPauseFailed --> ClusterReady: 恢复管理
    ClusterDryRunFailed --> ClusterReady: 重试成功
    ClusterDeleteFailed --> [*]: 最终状态
    
    ClusterUnknown: 未知 Phase (default 分支)
...
| 状态类别 | 设置位置 | 说明 |
| --------- | --------- | ------ |
| 安装/升级/扩缩容/删除/暂停/DryRun/Addons/纳管 | `phase_flow.go:calculateClusterStatusByPhase` | Phase 执行后根据成功/失败设置 |
| `ClusterReady` / `ClusterUnhealthy` / `ClusterChecking` | `ensure_cluster.go` | 健康检查逻辑设置 |
| `ClusterUnknown` | `phase_flow.go:351-353` | default 分支，未知 Phase 时设置 |

**完整状态转换矩阵**（来自 `calculateClusterStatusByPhase`）：
...
| default | `ClusterUnknown` | `ClusterUnknown` |

#### 2.2.2 安装状态切换规则

**位置**：[phase_flow.go:319-327](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/phase_flow.go#L319-L327)
...
- **Phase 列表**：`ClusterInitPhaseNames`
- **包含 Phase**：EnsureBKEAgent、EnsureNodesEnv、EnsureClusterAPIObj、EnsureCerts、EnsureLoadBalance、EnsureMasterInit、EnsureMasterJoin、EnsureWorkerJoin、EnsureAddonDeploy、EnsureNodesPostProcess、EnsureAgentSwitch

#### 2.2.3 集群状态映射

**位置**：[phase_flow.go:322-356](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/phase_flow.go#L322-L356)
...
| `ClusterScaleMasterDownPhaseNames` | ClusterMasterScalingDown | ClusterScaleFailed |
| `ClusterScaleWorkerDownPhaseNames` | ClusterWorkerScalingDown | ClusterScaleFailed |
| `ClusterUpgradePhaseNames` / `DeclarativeClusterUpgradePhaseNames` | ClusterUpgrading | ClusterUpgradeFailed |
| `ClusterDeletePhaseNames` | ClusterDeleting | ClusterDeleteFailed |
| `ClusterPausedPhaseNames` | ClusterPaused | ClusterPauseFailed |
| `ClusterDryRunPhaseNames` | ClusterDryRun | ClusterDryRunFailed |
| `ClusterAddonsPhaseNames` | ClusterDeployingAddon | ClusterDeployAddonFailed |
...
**位置**：[ensure_bke_agent.go:252-317](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_bke_agent.go#L252-L317)
...
**位置**：[ensure_nodes_env.go:257-314](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_nodes_env.go#L257-L314)
...
**集群状态切换**（来自 `phase_flow.go:calculateClusterStatusByPhase`）：

| 层次 | 状态 | 触发条件 | 设置位置 |
| ------ | ------ | ---------- | ---------- |
| 集群 | `ClusterInitializing` | 安装 Phase 执行中 | phase_flow.go:359 |
| 集群 | `ClusterInitializationFailed` | 安装 Phase 返回 error | phase_flow.go:361 |
| 集群 | `ClusterUpgrading` | 升级 Phase 执行中 | phase_flow.go:424 |
| 集群 | `ClusterUpgradeFailed` | 升级 Phase 返回 error | phase_flow.go:422 |
| 集群 | `ClusterMasterScalingUp` | Master 扩容 Phase 执行中 | phase_flow.go:370 |
| 集群 | `ClusterMasterScalingDown` | Master 缩容 Phase 执行中 | phase_flow.go:433 |
| 集群 | `ClusterWorkerScalingUp` | Worker 扩容 Phase 执行中 | phase_flow.go:379 |
| 集群 | `ClusterWorkerScalingDown` | Worker 缩容 Phase 执行中 | phase_flow.go:442 |
| 集群 | `ClusterScaleFailed` | 扩缩容 Phase 返回 error | phase_flow.go:368/377/431/440 |
| 集群 | `ClusterDeployingAddon` | Addon 部署 Phase 执行中 | phase_flow.go:415 |
| 集群 | `ClusterDeployAddonFailed` | Addon 部署 Phase 返回 error | phase_flow.go:413 |
| 集群 | `ClusterManaging` | 纳管 Phase 执行中 | phase_flow.go:451 |
| 集群 | `ClusterManageFailed` | 纳管 Phase 返回 error | phase_flow.go:449 |
| 集群 | `ClusterPaused` | 暂停 Phase 执行中 | phase_flow.go:397 |
| 集群 | `ClusterPauseFailed` | 暂停 Phase 返回 error | phase_flow.go:395 |
| 集群 | `ClusterDryRun` | DryRun Phase 执行中 | phase_flow.go:406 |
| 集群 | `ClusterDryRunFailed` | DryRun Phase 返回 error | phase_flow.go:404 |
| 集群 | `ClusterDeleting` | 删除 Phase 执行中 | phase_flow.go:388 |
| 集群 | `ClusterDeleteFailed` | 删除 Phase 返回 error | phase_flow.go:386 |
| 集群 | `ClusterReady` | 所有 Phase 成功 + 健康检查通过 | ensure_cluster.go |
| 集群 | `ClusterUnhealthy` | 健康检查失败 | ensure_cluster.go |
| 集群 | `ClusterChecking` | 健康检查中 | ensure_cluster.go |
| 集群 | `ClusterUnknown` | 未知 Phase (default 分支) | phase_flow.go:351-353 |

**集群健康状态切换**（ClusterHealthState）：

| 层次 | 状态 | 触发条件 | 设置位置 |
| ------ | ------ | ---------- | ---------- |
| 集群 | `Deploying`(HealthState) | 首次部署 | bkecluster_controller.go:765 |
| 集群 | `DeployFailed`(HealthState) | 失败超 10 次 | statusmanager.go:210 |
| 集群 | `Healthy`(HealthState) | 健康检查通过 | ensure_cluster.go |
| 集群 | `Unhealthy`(HealthState) | 健康检查失败 | ensure_cluster.go |
...
| 节点 | `NodeBootFlag`(StateCode) | WorkerJoin 成功 | ensure_worker_join.go |
...
**位置**：[ensure_master_init.go:523-528](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_master_init.go#L523-L528)
...
**位置**：[ensure_bke_agent.go:274-317](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_bke_agent.go#L274-L317)
...
**位置**：[ensure_nodes_env.go:285-314](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_nodes_env.go#L285-L314)
...
| ------ | ------ | ---------- | ---------- | ---------- |
| L1 | **controller-runtime workqueue 限速重试** | 单次 reconcile 失败 | FastSlowRateLimiter | [cmd/capbke/main.go:481-486](https://gitcode.com/openfuyao/cluster-api-provider-bke/cmd/capbke/main.go#L481-L486) |
| L2 | **StatusManager 失败计数 + 状态回滚** | BKECluster 状态级 | `ReconcileAllowedFailedCount` (默认10) | [pkg/statusmanage/statusmanager.go:175-215](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/statusmanage/statusmanager.go#L175-L215) |
| L3 | **RetryAnnotation 手动重试** | 失败节点级 | 注解 `annotation.RetryAnnotationKey` | [bkecluster_controller.go:660-738](https://gitcode.com/openfuyao/cluster-api-provider-bke/controllers/capbke/bkecluster_controller.go#L660-L738) |
...
| 字段 | 含义 | 取值范围 | 说明 |
|------|------|----------|------|
| **Phase** | 安装阶段名称 | - | 16 个安装 Phase 之一 |
| **BackoffLimit** | 重试次数限制 | 0-∞ | 0 表示不重试，由 StatusManager 统一管理重试逻辑 |
| **ActiveDeadlineSecond** | 超时时间（秒） | 60-1200 | Phase 执行的最大允许时间，超时后标记失败 |
| **BackoffDelay** | 重试间隔（秒） | 0-∞ | 两次重试之间的等待时间，0 表示立即重试 |
| **Requeue 策略** | 重试策略类型 | - | 自动重试/节点级重试/Command 级重试/不重试/快速失败 |

**重试策略说明**：

- **自动重试**：失败后自动重新入队，由 workqueue 限速器控制重试间隔
- **节点级重试**：单节点失败可重试，但不影响其他节点，适用于多节点场景
- **Command 级重试**：通过 Command CRD 的 BackoffLimit 控制重试次数
- **不重试**：失败后立即中断，需人工介入（如 EnsureMasterInit）
- **快速失败**：非关键操作，失败后快速跳过，不影响整体流程
...
BKE 安装流程采用**状态驱动**的幂等性设计，确保每个 Phase 在重试时能够：

1. **检测已完成状态**：通过位标记、Condition 或资源检查判断阶段是否已完成
2. **跳过已完成阶段**：已完成的 Phase 直接返回 `needExecute=false`，避免重复执行
3. **从失败点恢复**：失败的 Phase 重试时从失败点继续，而非从头开始
4. **避免副作用**：重复执行不会产生重复资源或状态冲突

#### 4.3.2 幂等机制分类

**1. 位标记机制（StateCode）**

使用 `BKENode.Status.StateCode` 的位标记记录阶段完成状态：

```go
// 位标记定义（api/capbke/v1beta1/bkecluster_consts.go）
const (
    NodeAgentPushedFlag   = 1 << iota  // bit 0 = 1    Agent 已推送
    NodeAgentReadyFlag                 // bit 1 = 2    Agent 健康检查通过
    NodeEnvFlag                        // bit 2 = 4    环境初始化完成
    NodeBootFlag                       // bit 3 = 8    引导成功
    NodeHAFlag                         // bit 4 = 16   高可用标记
    MasterInitFlag                     // bit 5 = 32   Master 初始化完成
    NodeDeletingFlag                   // bit 6 = 64   节点删除中
    NodeFailedFlag                     // bit 7 = 128  节点失败
    NodeStateNeedRecord                // bit 8 = 256  状态需要记录
    NodePostProcessFlag                // bit 9 = 512  后处理完成
...
// 设置位标记（ensure_bke_agent.go:295）
if err := nf.MarkNodeStateFlagForCluster(ctx, bkeCluster, node.IP, bkev1beta1.NodeAgentPushedFlag); err != nil {
    e.Ctx.Log.Warn(constant.InternalErrorReason, "Failed to mark node state flag: %v", err)
}

// 检查位标记（ensure_bke_agent.go:180）
...
使用 Kubernetes 标准 Condition 机制记录集群/节点状态：

```go
// 检查 Condition（ensure_master_init.go:540）
if conditions.IsTrue(e.Ctx.Cluster, clusterv1.ControlPlaneInitializedCondition) {
    return false  // 控制面已初始化，跳过
}

// 设置 Condition（ensure_master_init.go:528）
conditions.MarkTrue(e.Ctx.Cluster, clusterv1.ControlPlaneInitializedCondition)
```

**3. 资源存在性检查**

检查 Secret、ConfigMap 等资源是否已存在：

```go
// 检查 Secret 是否存在（ensure_certs.go）
secret := &corev1.Secret{}
if err := c.Get(ctx, types.NamespacedName{Name: "bke-certs", Namespace: bkeCluster.Namespace}, secret); err == nil {
    return false  // Secret 已存在，跳过
}

// 检查 CRD 对象是否存在（ensure_cluster_api_obj.go）
cluster := &clusterv1.Cluster{}
if err := c.Get(ctx, types.NamespacedName{Name: bkeCluster.Name, Namespace: bkeCluster.Namespace}, cluster); err == nil {
    return false  // Cluster 对象已存在，跳过
}
```
...
```bash
# kubeadm init 使用 --ignore-preflight-errors=all 忽略已存在的检查
kubeadm init --ignore-preflight-errors=all

# kubeadm join 使用 --ignore-preflight-errors=all 忽略已加入的检查
kubeadm join --ignore-preflight-errors=all
```

#### 4.3.3 各 Phase 幂等机制汇总

| Phase | 幂等机制 | 检查方式 | 代码位置 |
| ------- | ---------- | ---------- | ---------- |
| EnsureBKEAgent | `NodeAgentPushedFlag` 位标记 | `StateCode & NodeAgentPushedFlag != 0` | ensure_bke_agent.go:180 |
| EnsureNodesEnv | `NodeEnvFlag` 位标记 | `StateCode & NodeEnvFlag != 0` | ensure_nodes_env.go:257 |
| EnsureMasterInit | `ControlPlaneInitializedCondition` | `conditions.IsTrue()` | ensure_master_init.go:540 |
| EnsureMasterJoin | `NodeBootFlag` 位标记 | `StateCode & NodeBootFlag != 0` | ensure_master_join.go:xxx |
| EnsureWorkerJoin | `NodeBootFlag` 位标记 | `StateCode & NodeBootFlag != 0` | ensure_worker_join.go:xxx |
| EnsureCerts | Secret 存在性检查 | `c.Get() == nil` | ensure_certs.go:xxx |
| EnsureClusterAPIObj | CRD 对象存在性检查 | `c.Get() == nil` | ensure_cluster_api_obj.go:xxx |
| EnsureLoadBalance | ConfigMap 存在性检查 | `c.Get() == nil` | ensure_load_balance.go:xxx |
| EnsureAddonDeploy | Addon 状态检查 | `bkeCluster.Status.AddonStatus` | ensure_addon_deploy.go:xxx |
| EnsureNodesPostProcess | `NodePostProcessFlag` 位标记 | `StateCode & NodePostProcessFlag != 0` | ensure_nodes_postprocess.go:xxx |
| EnsureAgentSwitch | Agent 监听目标检查 | `bkeagent.GetListenTarget()` | ensure_agent_switch.go:xxx |
...
**位置**：[api/bkecommon/v1beta1/bkecluster_spec.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/api/bkecommon/v1beta1/bkecluster_spec.go)
...
| `spec.pause` | `bool` | 暂停集群管理（触发 EnsurePaused Phase） | `true` |

#### 5.1.1 spec.pause 字段详解

`spec.pause` 是 BKECluster 的声明式控制字段，用于暂停或恢复集群管理。

**作用机制**：
1. 用户修改 `spec.pause` 字段
2. EnsurePaused Phase 检测到 `spec.pause` 与内部注解不一致
3. EnsurePaused Phase 执行，同步暂停状态到相关资源：
   - 设置 `bke.bocloud.com/pause` 注解
   - 暂停所有 Command（设置 `cmd.Spec.Suspend=true`）
   - 暂停 Cluster API 对象（KubeadmControlPlane、MachineDeployment）

**代码位置**：
- 字段定义：[api/bkecommon/v1beta1/bkecluster_spec.go:48](https://gitcode.com/openfuyao/cluster-api-provider-bke/api/bkecommon/v1beta1/bkecluster_spec.go#L48)
- 执行逻辑：[pkg/phaseframe/phases/ensure_paused.go:59-100](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/phaseframe/phases/ensure_paused.go#L59-L100)

### 5.2 安装运行时控制（BKECluster Annotations）

**位置**：[utils/capbke/annotation/helper.go](https://gitcode.com/openfuyao/cluster-api-provider-bke/utils/capbke/annotation/helper.go)

| 注解 | 说明 | 示例 |
| ------ | ------ | ------ |
| `bke.bocloud.com/retry` | 手动重试失败节点 | `""`（全部）或 `"10.0.0.1,10.0.0.2"`（指定节点） |
| `bke.bocloud.com/pause` | 暂停状态同步标记（由控制器自动设置） | `"true"` |
| `bke.bocloud.com/deep-restore-node` | 节点环境初始化时的清理范围控制 | `"true"`（深度清理）或 `"false"`（浅度清理） |

#### 5.2.1 spec.pause 与 bke.bocloud.com/pause 注解的区别

| 维度 | spec.pause | bke.bocloud.com/pause 注解 |
|------|-------------|---------------------------|
| **类型** | BKECluster Spec 字段（布尔值） | BKECluster Annotation（字符串） |
| **用途** | **声明式控制**：用户通过修改此字段表达暂停/恢复意图 | **内部同步机制**：控制器用于同步暂停状态到相关资源 |
| **设置方式** | 用户直接修改 BKECluster YAML | 由 EnsurePaused Phase 自动设置 |
| **作用范围** | 触发 EnsurePaused Phase 执行 | 同步到 Command、Cluster API 对象等 |

**工作流程**：

```
用户设置 spec.pause=true
    ↓
EnsurePaused.NeedExecute() 检测到 spec.pause 与注解不一致
    ↓
EnsurePaused.Execute() 执行
    ↓
syncBKEClusterPauseStatus() 设置 bke.bocloud.com/pause="true" 注解
    ↓
pauseOrResumeCommands() 暂停所有 Command（设置 cmd.Spec.Suspend=true）
    ↓
pauseOrResumeClusterAPIObj() 暂停 Cluster API 对象（KCP、MD）
```

**使用建议**：
- 用户应通过修改 `spec.pause` 字段来控制暂停状态
- `bke.bocloud.com/pause` 注解由控制器自动管理，不建议手动修改

#### 5.2.2 bke.bocloud.com/deep-restore-node 注解详解

**作用**：控制节点环境初始化（EnsureNodesEnv Phase）时的清理范围。

**清理范围对比**：

| 注解值 | 清理范围 | 说明 |
|--------|---------|------|
| `"true"`（默认） | `cert,manifests,container,kubelet,containerRuntime,extra` | **深度清理**：包括容器运行时（containerd/docker） |
| `"false"` | `cert,manifests,container,kubelet,extra` | **浅度清理**：不包括容器运行时 |

**代码实现**（[pkg/command/env.go:149-155](https://gitcode.com/openfuyao/cluster-api-provider-bke/pkg/command/env.go#L149-L155)）：
...
**使用场景**：
- **深度清理**（默认）：节点环境严重损坏，需要完全重置，包括容器运行时
- **浅度清理**：仅重置 Kubernetes 相关组件，保留容器运行时配置（适用于容器运行时配置复杂或自定义的场景）

**默认值设置**（[utils/capbke/annotation/helper.go:82-84](https://gitcode.com/openfuyao/cluster-api-provider-bke/utils/capbke/annotation/helper.go#L82-L84)）：
...
```bash
# 启用深度清理（默认行为）
kubectl annotate bkecluster my-cluster bke.bocloud.com/deep-restore-node="true"

# 禁用深度清理（保留容器运行时配置）
kubectl annotate bkecluster my-cluster bke.bocloud.com/deep-restore-node="false"
```
...
graph TD
    ...
    CheckDisk --> CheckResource[检查资源 CPU 和内存]
