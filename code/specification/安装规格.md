...
- [1. 安装场景规格](#1-安装场景规格)
  ...
  - [1.2 各安装场景规格](#12-各安装场景规格)
  - [1.3 安装场景汇总](#13-安装场景汇总)
  - [1.4 关键设计约束](#14-关键设计约束)
- [2. 安装状态机规格](#2-安装状态机规格)
  - [2.1 状态定义](#21-状态定义)
...
#### 1.2.1 EnsureFinalizer（添加 Finalizer 保护）
...
#### 1.2.3 EnsureClusterManage（纳管判断）
...
#### 1.2.4 EnsureDeleteOrReset（删除/重置判断）
...
#### 1.2.5 EnsureDryRun（DryRun 试运行模式）
...
#### 1.2.8 EnsureClusterAPIObj（Cluster API 对象创建）
...
#### 1.2.10 EnsureLoadBalance（负载均衡配置）
...
#### 1.2.11 EnsureMasterInit（Master 初始化）
...
#### 1.2.12 EnsureMasterJoin（Master 加入）
...
#### 1.2.13 EnsureWorkerJoin（Worker 加入）
...
#### 1.2.14 EnsureAddonDeploy（Addon 组件部署）
...
#### 1.2.15 EnsureNodesPostProcess（节点后置处理）
...
#### 1.2.16 EnsureAgentSwitch（Agent 监听模式切换）
