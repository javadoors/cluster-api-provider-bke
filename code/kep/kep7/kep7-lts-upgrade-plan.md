...
bkeadm 一键升级命令的设计目标是提供简单、安全、可控的 LTS 版本升级体验。设计原则：

1. **简单性**: 用户只需指定源版本和目标版本，无需关心中间步骤
2. **安全性**: 在执行前进行严格的版本校验，确保升级路径合法
4. **可控性**: 支持 dry-run 模式预览升级路径，支持跳过预处理步骤

**命令架构**

```
┌─────────────────────────────────────────────────────────────────┐
│                    bkeadm upgrade lts 命令架构                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  用户输入: bkeadm upgrade lts --from 25.12 --to 26.12          │
│                                                                 │
│  ┌─────────────────────────────────────────────────────────┐   │
│  │  1. 参数解析层                                          │   │
│  │     - 解析 --from, --to, --skip-preprocessing, --dry-run│   │
│  │     - 参数校验                                           │   │
│  └─────────────────────────────────────────────────────────┘   │
│                          │                                      │
│                          ▼                                      │
...
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  UpgradePath CR                                 │   │   │
│  │  │  versions: [25.12(LTS), 26.03, 26.06, 26.09,   │   │   │
│  │  │            26.12(LTS)]                          │   │   │
│  │  │  paths: [25.12->26.03, 26.03->26.06, ...]      │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  │                     │                                   │   │
│  │                     ▼                                   │   │
│  │  ┌─────────────────────────────────────────────────┐   │   │
...
│  │  Step 2: 获取升级路径                                   │   │
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  ① 从 UpgradePath CR 解析 Hop 列表              │   │   │
│  │  │     → [25.12→26.03, 26.03→26.06,               │   │   │
...
bkeadm upgrade lts --from 25.12 --to 26.12
  ...
  |
  +-> Step 2: 获取升级路径
  |     - 从 UpgradePath CR 中获取升级路径定义，解析出每个 Hop (from -> to)
  |     - 对每个 Hop，从 Registry 中查找对应的处理器 (Handler)
  |     - 如果某个 Hop 在 Registry 中未注册处理器，返回错误
  |
  +-> Step 3: 逐 Hop 执行升级
  |     |
...
升级版本处理器框架采用**策略模式 (Strategy Pattern)** 设计，核心设计原则：

1. **接口抽象**: 定义统一的 Handler 接口，所有版本跳转处理器实现相同接口
2. **注册表模式**: 使用 Registry 管理所有处理器，支持动态注册和查找
3. **路径遍历**: Registry 提供路径遍历能力，自动查找从源版本到目标版本的完整路径
4. **可扩展性**: 新增版本跳转只需实现 Handler 接口并注册到 Registry
...
│  │  │  FromVersion() string                           │   │   │
│  │  │  ToVersion() string                             │   │   │
│  │  │  PreProcess(ctx) error                          │   │   │
│  │  │  Upgrade(ctx) error                             │   │   │
│  │  │  PostProcess(ctx) error                         │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────┘   │
│                          ▲                                      │
│                          │ 实现                                 │
...
│  │                                                         │   │
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  GetUpgradePath(from, to)                       │   │   │
│  │  │  → 遍历 handlers，查找完整路径                  │   │   │
│  │  │  → 返回 [Handler1, Handler2, ...]               │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────┘   │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
...
每个 Hop 处理器实现 Handler 接口，采用**三阶段执行模型**：

1. **PreProcess (预处理)**: 执行升级前的准备工作，如 CRD 迁移、API 兼容性处理、数据迁移等
2. **Upgrade (升级)**: 执行实际的升级操作，可以是 PhaseFlow 升级或声明式升级
3. **PostProcess (后处理)**: 执行升级后的验证工作，如验证集群状态、验证组件健康等

**设计原则**：

1. **职责分离**: 每个阶段职责明确，便于维护和测试
2. **可组合性**: 不同 Hop 可以组合不同的升级策略 (PhaseFlow/声明式)
3. **错误处理**: 每个阶段独立处理错误，支持回滚和重试
4. **可扩展性**: 新增 Hop 只需实现 Handler 接口，无需修改框架代码
...
升级编排器采用**编排者模式 (Orchestrator Pattern)** 设计，负责协调整个升级流程的执行。设计原则：

1. **流程编排**: Orchestrator 负责协调整个升级流程，包括获取升级路径、校验版本、逐 Hop 执行升级
2. **依赖注入**: 通过构造函数注入依赖 (client, registry, dryRun)，便于测试和扩展
3. **错误处理**: 每个步骤独立处理错误，支持快速失败和错误传播
4. **Dry-Run 支持**: 支持 dry-run 模式，可以预览升级路径而不实际执行

**编排器职责**：

1. **获取升级路径**: 从 UpgradePath CR 中获取升级路径定义
2. **版本校验**: 验证输入版本的合法性 (LTS 版本、版本存在性、路径有效性)
3. **路径解析**: 解析从源版本到目标版本的完整升级路径
4. **逐 Hop 执行**: 逐个执行每个 Hop 的升级操作 (PreProcess -> Upgrade -> PostProcess)
...
│  │  ┌─────────────────────────────────────────────────┐   │   │
│  │  │  Methods                                        │   │   │
│  │  │  ├── Execute(ctx, from, to) error               │   │   │
│  │  │  ├── getUpgradePath(ctx) (*UpgradePath, error)  │   │   │
│  │  │  ├── validateVersions(path, from, to) error     │   │   │
│  │  │  └── parseUpgradePath(path, from, to) ([]Hop)   │   │   │
│  │  └─────────────────────────────────────────────────┘   │   │
│  └─────────────────────────────────────────────────────────┘   │
│                          │                                      │
│                          ▼                                      │
...
bkeadm 命令入口采用**薄命令层 (Thin Command Layer)** 设计，将命令行解析与业务逻辑分离：

1. **职责分离**: 命令入口只负责参数解析、处理器注册、编排器调用，不包含具体的升级逻辑
2. **处理器集中注册**: 所有 Hop 处理器在命令入口统一注册，支持从 UpgradePath CR 动态加载
3. **统一入口**: 所有 LTS 升级场景都通过 `bkeadm upgrade lts` 命令统一入口，避免分散的命令
4. **错误传播**: 命令入口将底层错误统一包装，返回清晰的错误信息给用户

**职责划分**：

| 组件 | 职责 | 不负责 |
|------|------|--------|
| **命令入口** | 参数解析、处理器注册、编排器调用 | 升级逻辑、版本校验、路径解析 |
| **编排器 (Orchestrator)** | 获取升级路径、版本校验、逐 Hop 执行 | 具体 Hop 的预处理/升级/后处理逻辑 |
| **处理器 (Handler)** | 单个 Hop 的 PreProcess/Upgrade/PostProcess | 升级路径规划、版本校验 |
