...
#### 6.1.2 ClusterComponent 不是 Operator — 概念澄清
...
RelatedObjects 声明组件管理的相关 Kubernetes 资源列表，是组件与外部工具之间的**资源发现契约**。它解决的核心问题是："这个组件在集群上创建了哪些资源？出了问题该收集哪些资源来诊断？"

| 作用 | 说明 | 没有 RelatedObjects 时 |
|------|------|----------------------|
| **must-gather 诊断** | must-gather 按列表定向收集组件相关资源 (YAML/日志/事件)，而非全集群扫描 | must-gather 不知该收集哪些资源，只能全量导出或遗漏关键资源 |
| **insights/巡检** | 巡检工具按列表检查组件资源是否健康、是否有配置 drift | 巡检工具无法知道组件管理哪些资源，无法做定向检查 |
| **CVO 资源追踪** | CVO 通过列表知道组件管理哪些资源，支持 prune (裁剪废弃资源) 和 overrides (标记非管理) | CVO 无法区分组件管理的资源和用户手动创建的资源，prune 可能误删 |
| **运维定位** | 管理员通过 `kubectl describe clustercomponent coredns` 查看相关资源，快速定位问题 | 管理员需自行猜测组件创建了哪些资源，排查效率低 |
| **依赖可视化** | 平台 UI/API 可按列表渲染组件资源拓扑图，展示组件的资源依赖关系 | 无法可视化组件资源拓扑 |

#### 20.7.2 设计思路

RelatedObjects 的设计借鉴 OpenShift ClusterOperator 的 `status.relatedObjects` 字段，核心思路是**让组件自身声明管理的资源，而非由外部工具猜测**：
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  传统模式 (无 RelatedObjects):                                                   │
│  ┌─────────────┐          ┌──────────────────────┐                             │
│  │ must-gather │ ──全量──→ │ 集群所有资源          │ ← 大量无关资源，收集慢      │
│  │ 巡检工具    │ ──扫描──→ │ (逐个检查)            │ ← 无法定向，效率低         │
│  │ 管理员      │ ──猜测──→ │ "coredns 创建了什么?" │ ← 可能遗漏                   │
│  └─────────────┘          └──────────────────────┘                             │
│                                                                                 │
│  CVO 设计模式 (有 RelatedObjects):                                              │
│  ┌─────────────┐          ┌──────────────────────┐                             │
│  │ Component   │  声明    │ relatedObjects:       │                             │
│  │ Executor    │ ───────→ │ - namespaces/kube-sys │                             │
│  │ (组件自身)  │          │ - deployments/coredns  │                             │
│  │             │          │ - services/kube-dns    │                             │
│  │             │          │ - configmaps/coredns-c │                             │
│  └─────────────┘          └──────────┬───────────┘                             │
│                                        │ 定向消费                                │
│  ┌─────────────┐                     │                                         │
│  │ must-gather │ ←───定向收集────────┘ ← 仅收集相关资源，快且准               │
│  │ 巡检工具    │ ←───定向检查──────────┘ ← 仅检查声明资源                     │
│  │ 管理员      │ ←───describe 查看────┘ ← 一目了然                            │
│  │ CVO prune  │ ←───定向裁剪──────────┘ ← 仅裁剪声明资源                     │
│  └─────────────┘                                                               │
│                                                                                 │
│  核心原则:                                                                       │
│  • 组件自身最清楚自己创建了哪些资源 → 由组件声明 (去中心化)                     │
│  • 外部工具按声明定向消费 (不需要猜测) → 解耦                                   │
│  • 声明可动态更新 (运行时反映实际状态) → 准确                                   │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
| 设计原则 | 说明 |
|---------|------|
| **组件自声明** | 组件执行器最清楚自己创建了哪些资源，由执行器在运行时更新 relatedObjects，而非由 CVO 猜测 |
| **去中心化** | 每个组件独立声明自己的资源，CVO 不需要为每个组件维护资源列表 |
| **动态更新** | 执行器应在运行时动态更新 relatedObjects，反映实际管理的对象 (如根据配置动态创建/删除资源) |
| **通配符支持** | 支持命名空间级通配 (name="" 表示命名空间内全部) 和集群级通配 (namespace="" name="") |
| **资源发现契约** | relatedObjects 是组件与外部工具之间的契约，外部工具信任声明的完整性 |
...
| 规则 | 说明 | 违反后果 |
|------|------|---------|
| 必须声明 namespace | 至少声明组件所在 namespace | must-gather 不知该收集哪个命名空间的资源 |
| 必须声明至少一个非 namespace 对象 | 如 Deployment/Service/ConfigMap | must-gather 仅收集命名空间，遗漏组件实际资源 |
| 执行器应在运行时动态更新 | 确保反映实际管理的对象 | relatedObjects 过时 → 诊断收集到错误资源或遗漏 |
| 通配符谨慎使用 | 仅在确实需要收集全部同类资源时使用 | 通配符过宽 → must-gather 收集大量无关资源 |
| 不要声明不属于自己的资源 | 仅声明组件创建/管理的资源 | 误声明 → 其他组件资源被错误收集或 prune |
| group 使用 API group (非 kind) | 如 `apps` (非 `Deployment`)，`""` 表示核心组 | group 错误 → 资源无法被发现 |
| resource 使用复数资源名 | 如 `deployments` (非 `deployment`)，`configmaps` (非 `configmap`) | 单数名 → 资源无法被发现 |
