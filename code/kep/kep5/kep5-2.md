...
```mermaid
flowchart TD
    Caller["Caller Reconciler"] -->|"GetComponentManifests name, version, ctx"| MS["ManifestStore"]
    ...
    subgraph MS["ManifestStore"]
        ...
        Exists{"存在?"}
        ScanFiles["扫描所有 .yaml 文件"]
        ParseMeta["解析 component.yaml 元数据"]
        ParseManifests["解析其余 YAML 为资源清单列表"]
        
        Step3["3. OCI 远程拉取<br/>registry/openfuyao-release:version"]
        PullOCI["拉取镜像 提取组件目录 layer"]
        CacheOCI["写入缓存"]
        Fail["返回错误"]
        
        Step4["4. 模板渲染 若传入 ctx"]
        RenderMeta["渲染 component.yaml 元数据"]
        SortManifests["按依赖顺序排序 CRD→RBAC→CM→Deploy"]
        RenderFiles["逐个渲染 .xxx 占位符"]
        Return["返回渲染后的清单列表"]
    end
    ...
    Hit -->|是| Clone --> Step4
    Hit -->|否| Step2 --> Exists
    Exists -->|是| ScanFiles --> ParseMeta --> ParseManifests --> Step4
    Exists -->|否| Step3 --> PullOCI
    PullOCI -->|失败| Fail
    PullOCI -->|成功| CacheOCI --> Step4
    Step4 --> RenderMeta --> SortManifests --> RenderFiles --> Return
```
...
```mermaid
flowchart TD
    A["BKEClusterReconciler.Reconcile"] --> B["1. 获取 BKECluster 实例"]
    B --> C["2. 检查 upgrade-ready Annotation"]
    C --> D["3. 构建 VersionContext"]
    D --> E["4. 构建 TemplateContext 调用位置1"]
    E --> F["5. 解析 ReleaseImage"]
    F --> G["6. 构建 DAG"]
    G --> H["7. executeDAG 调用位置2"]
