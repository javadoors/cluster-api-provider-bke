...
```mermaid
graph TD
    BKECluster["BKECluster<br/>(集群实例，生命周期管理)"]
    CV["ClusterVersion<br/>spec.desiredVersion: v2.6.0<br/>status.currentVersion: v2.5.0<br/>status.currentReleaseImageRef: ri-v2.5.0"]
    RI["ReleaseImage<br/>spec.version: v2.6.0<br/>spec.ociRef: registry/openfuyao-release:v2.6.0<br/>spec.install.components: [{name: k8s, ver: v1.29.0}, ...]<br/>spec.upgrade.components: [{name: provider, ver: v1.2.0}, ...]"]
    BM["bke-manifests (ComponentVersion 定义)<br/>bke-manifests/kubernetes/v1.29.0/component.yaml<br/>bke-manifests/provider-upgrade/v1.0.0/component.yaml"]
    UP["UpgradePath<br/>spec.ociRef: registry/openfuyao-upgradepath:latest<br/>paths: [{from: v2.5.0, to: v2.6.0, blocked: false}]"]

    BKECluster -->|"1:1 (OwnerReference)"| CV
    CV -->|"1:1 引用"| RI
    RI -->|"按 (name,version) 定位"| BM
```
...
```mermaid
flowchart TD
    Start(["BKEClusterReconciler.buildVersionContext(bc)"]) --> Step1["1. 获取 ClusterVersion<br/>cv := GetClusterVersionsForBKECluster(bc)"]
    
    Step1 --> Step2["2. 构建 Target 版本映射"]
    Step2 --> GetDesired["desiredVer := cv.Spec.DesiredVersion"]
    GetDesired --> GetTargetRI["targetRI := manifestStore.GetReleaseImage(desiredVer)"]
    GetTargetRI --> LoopTargetUpgrade["for each component in targetRI.Spec.Upgrade:<br/>  vCtx.SetTarget(component.Name, component.Version)"]
    LoopTargetUpgrade --> LoopTargetInstall["for each component in targetRI.Spec.Install:<br/>  vCtx.SetTarget(component.Name, component.Version)"]
    
    LoopTargetInstall --> Step3["3. 构建 Current 版本映射"]
    Step3 --> GetCurrent["currentVer := cv.Status.CurrentVersion"]
    GetCurrent --> GetCurrentRI["currentRI := manifestStore.GetReleaseImage(currentVer)"]
    GetCurrentRI --> LoopCurrentUpgrade["for each component in currentRI.Spec.Upgrade:<br/>  vCtx.SetCurrent(component.Name, component.Version)"]
    LoopCurrentUpgrade --> LoopCurrentInstall["for each component in currentRI.Spec.Install:<br/>  vCtx.SetCurrent(component.Name, component.Version)"]
    
    LoopCurrentInstall --> Step4["4. 注入到 PhaseContext<br/>bc.PhaseContext.VersionContext = vCtx"]
...
flowchart TD
    subgraph RI["ReleaseImage 升级组件声明"]
        direction TB
        Comp1["pre-upgrade-resources ◀──── 新增组件<br/>version: v1.0.0"]
        Comp2["provider-upgrade<br/>version: v1.2.0"]
        Comp3["etcd-upgrade<br/>version: v3.5.12"]
        
        DAG["DAG 执行顺序:<br/>pre-upgrade-resources → provider-upgrade → etcd-upgrade"]
    end
