...
## 1. 摘要
本提案引入 `ClusterVersion`、`ReleaseImage` 与 `UpgradePath` 三个核心 CRD，借鉴 OpenShift CVO 的声明式版本管理理念，结合 `bke-manifests` 作为组件清单物理载体，实现 openFuyao 集群的版本升级。方案保持现有 `BKEClusterReconciler` 的 Phase 编排逻辑不变，通过注入版本比对上下文，使 `EnsureProviderSelfUpgrade`、`EnsureAgentUpgrade`、`EnsureComponentUpgrade`、`EnsureEtcdUpgrade` 具备声明式升级决策能力。版本声明、清单校验、路径管控与执行编排解耦，支持组件独立演进与平滑迁移。
...
5. 在指定 Phase 中注入版本比对逻辑，实现“有变更则升级，无变更则跳过”的幂等执行。
...
│  spec.version: v2.6.0                                          │
│  spec.manifestRef: bke-manifests:v2.6.0                        │
│  spec.components:                                              │
│    - name: etcd          ────────┐                             │
│      version: v3.5.12            │                             │
...
│  spec.fromVersion: v2.5.0                                        │
│  spec.toVersion: v2.6.0                                          │
│  spec.blocked: false                                             │
│  spec.preCheck / spec.postCheck                                  │
└──────────────────────────────────────────────────────────────────┘
```
...
type ReleaseImageSpec struct {
    ...
    ManifestRef  string          `json:"manifestRef"` // bke-manifests 镜像 tag
...
type UpgradePathSpec struct {
    ...
    PostCheck   *HookSpec `json:"postCheck,omitempty"`
}
```

### 4.3 升级路径设计方案
`UpgradePath` 独立于 `ReleaseImage` 演进，支持动态添加前置检查、拦截高风险升级路径、标记废弃路径。
- **查找策略**：`ClusterVersion` 变更时，控制器按 `{from}-to-{to}` 命名规则或 Label 选择器查找 `UpgradePath`。
- **拦截机制**：若 `spec.blocked=true`，升级流程中止并记录 `UpgradeBlocked` 事件。
- **兼容检查**：`PreCheck` 可声明脚本或 API 调用，验证节点状态、磁盘空间、组件健康度。
- **独立演进**：发现问题后仅需更新 `UpgradePath` CR，无需重新构建 `bke-manifests` 或 `ReleaseImage`。

### 4.4 控制器架构与职责划分
| 控制器 | 核心职责 | 协同方式 |
|--------|---------|----------|
| **ClusterVersionReconciler** | 版本声明管理、ReleaseImage 引用维护、升级路径校验、触发 BKECluster 调谐 | 更新 BKECluster Annotation `cvo.openfuyao.cn/upgrade-ready` |
| **ReleaseImageReconciler** | 清单校验、组件路径验证、bke-manifests 可用性检查、状态标记 | 独立调谐，更新 Status.Phase = Valid/Invalid |
...
### 4.5 Phase 集成逻辑（需求 9c 实现）
保持原有 Phase 逻辑不变，注入 `VersionContext` 与 `ShouldUpgrade` 决策器：
```go
// pkg/upgrade/context.go
type VersionContext struct {
    CurrentComponents map[string]string // name -> version
    TargetComponents  map[string]string
}

// 在 Phase 入口注入
func (p *EnsureProviderSelfUpgrade) ShouldUpgrade(ctx *VersionContext) bool {
    cur := ctx.CurrentComponents["bke-controller-manager"]
    tgt := ctx.TargetComponents["bke-controller-manager"]
    return cur != "" && cur != tgt
}

func (p *EnsureProviderSelfUpgrade) Execute() (ctrl.Result, error) {
    if !p.ShouldUpgrade(p.VersionCtx) {
        p.Log.Info("Component version unchanged, skipping upgrade")
...
│  • 解析目标 ReleaseImage                                    │
│  • 调用 UpgradePath.Find() 校验路径合法性                   │
│  • 校验 ReleaseImage.Status == Valid                        │
...
│  • ShouldUpgrade(ctx) ? Yes → 执行原有逻辑                  │
│  • ShouldUpgrade(ctx) ? No  → 跳过，标记 Succeeded          │
...
| **2. 清单解析层** | `bke-manifests` 挂载适配、`ComponentVersion` 路径解析、Store 接口 | 5 | 兼容现有镜像目录结构 |
| **3. 控制器开发** | CV/RI/UP 控制器核心逻辑、状态机、事件协同机制 | 10 | 独立调谐，解耦设计 |
| **4. BKECluster 增强** | 差异计算模块、依赖排序、Phase 桥接、升级历史记录 | 10 | 核心编排逻辑，保证幂等 |
| **5. Phase 适配** | 4 个 Phase 注入 `VersionContext` 与 `ShouldUpgrade` 逻辑 | 6 | 保持原有逻辑，仅替换入口决策 |
...
| 风险 | 影响 | 缓解措施 |
|------|------|---------|
| `bke-manifests` 拉取失败或路径错误 | 升级阻塞 | 控制器增加健康检查与重试；`ReleaseImage` 提前标记 Invalid；支持本地缓存 fallback |
...
| 测试类型 | 覆盖场景 |
|----------|---------|
| **单元测试** | `CalculateComponentDiff` 逻辑、依赖排序算法、`ShouldUpgrade` 决策、路径拦截 |
| **集成测试** | `ClusterVersion` ↔ `ReleaseImage` ↔ `BKECluster` 状态联动、Annotation 触发机制 |
| **E2E 测试** | 补丁升级、跨版本升级、单组件独立升级、失败中断与回滚、`bke-manifests` 缺失降级 |
...
4. **Phase 覆盖**：4 个指定 Phase 均被正确调度，`ShouldUpgrade` 逻辑生效。
