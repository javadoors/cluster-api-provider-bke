...
`ExecuteDAG` 函数是**声明式升级 DAG 调度器的核心执行函数，负责按拓扑顺序编排和执行升级组件**。

## **核心作用**

ExecuteDAG 函数的主要职责是：
1. **拓扑排序**：将 DAG 转换为批次（Batch），确保依赖关系正确
2. **批次执行**：按批次顺序执行组件，批次内并行，批次间串行
3. **错误处理**：收集错误并根据 FailurePolicy 决定是否停止后续批次
4. **状态持久化**：标记组件完成或失败状态

## **Scheduler 结构**

[scheduler.go:44-49](file:///\cluster-api-provider-bke\pkg\dagexec\scheduler.go#L44-L49)
```go
type Scheduler struct {
    InlineRunner          InlinePhaseRunner      // inline 类型组件执行器
    ManifestStore         manifest.Store         // manifest 类型组件存储
    ManifestApplier       manifest.Applier       // manifest 应用器
    MaxParallelPerBatch   int                    // 批次内最大并行数（默认 8）
}
```

## **ExecuteDAG 执行流程**

[scheduler.go:81-132](file:///\cluster-api-provider-bke\pkg\dagexec\scheduler.go#L81-L132)
```go
func (s *Scheduler) ExecuteDAG(
    ...
    tmpl := manifest.TemplateContext{
        ...
        OpenFuyaoVersion: spec.OpenFuyaoVersion,
    }
...
假设 DAG 结构：
```
Batch 0: [PreUpgradeResources]  // 无依赖，第一批
Batch 1: [AgentUpgrade, ContainerdUpgrade]  // 依赖 PreUpgradeResources
Batch 2: [EtcdUpgrade]  // 依赖 ContainerdUpgrade
Batch 3: [MasterUpgrade, WorkerUpgrade]  // 依赖 EtcdUpgrade
```

### **批次内并行执行**

[scheduler.go:133-180](file:///\cluster-api-provider-bke\pkg\dagexec\scheduler.go#L133-L180)
```go
func (s *Scheduler) executeBatchParallel(...) (batchErrs []error, failFastStop bool) {
    // 1. 筛选需要执行的组件
    var items []workItem
    ...
    parallelLimit := s.maxParallel(len(items))  // 默认 8
...
[scheduler.go:326-337](file:///\cluster-api-provider-bke\pkg\dagexec\scheduler.go#L326-L337)
```go
func (s *Scheduler) executeComponent(...) error {
    // 根据组件类型选择执行路径
    if node.Inline != nil {
        // inline 类型：通过 PhaseRunner 执行
        return s.executeInline(phaseCtx, oldCluster, newCluster, node)
    }
    // manifest 类型：通过 ManifestApplier 应用 YAML
...
[scheduler.go:339-356](file:///\cluster-api-provider-bke\pkg\dagexec\scheduler.go#L339-L356)
```go
func (s *Scheduler) executeInline(...) error {
    handler := node.Inline.Handler  // 如 "EnsureEtcdUpgrade"
    version := node.Inline.Version  // 如 "manifest"
...
[scheduler.go:358-380](file:///\cluster-api-provider-bke\pkg\dagexec\scheduler.go#L358-L380)
```go
func (s *Scheduler) executeManifest(...) error {
    // 1. 从 ManifestStore 获取组件 YAML
    pkg, err := s.ManifestStore.GetComponentManifests(node.Name, node.Version)
    if err != nil {
        return err
    }
    ...
    if len(pkg.Manifests) == 0 {
        return fmt.Errorf("no manifests for component %s@%s", node.Name, node.Version)
    }
    
    // 3. 通过 ManifestApplier 应用到集群
    return s.ManifestApplier.ApplyComponent(ctx, phaseCtx, pkg, tmpl)
}
```

## **状态持久化**

[scheduler.go:220-248](file:///\cluster-api-provider-bke\pkg\dagexec\scheduler.go#L220-L248)
```go
func (s *Scheduler) persistBatchResults(...) ([]error, bool) {
...
### **4. FailFast 策略**
- 某个组件失败时，根据 `FailurePolicy` 决定是否停止后续批次
- `FailFast`：立即停止，返回错误
- `Continue`：记录错误，继续下一批次

### **5. 错误聚合**
- 使用 `kerrors.NewAggregate` 收集所有错误
- 返回完整的错误列表
...
ExecuteDAG
  ↓
拓扑排序 → Batches: [[PreUpgrade], [Agent, Containerd], [Etcd], [Master, Worker]]
  ↓
Batch 0: [PreUpgradeResources]
  ↓
  executeBatchParallel (并行数=1)
    ↓
    executeComponent(PreUpgrade)
      ↓
      InlineRunner.Execute("EnsurePreUpgradeResources", "manifest")
        ↓
        Phase.Execute() → 创建升级前资源
      ↓
    markComponentCompleted(PreUpgrade) → 标记完成
  ↓
Batch 1: [AgentUpgrade, ContainerdUpgrade]
  ↓
  executeBatchParallel (并行数=8)
    ↓
    并行执行:
      - executeComponent(Agent) → InlineRunner.Execute("EnsureAgentUpgrade")
      - executeComponent(Containerd) → InlineRunner.Execute("EnsureContainerdUpgrade")
    ↓
    等待所有完成
    ↓
    markComponentCompleted(Agent, Containerd)
  ↓
Batch 2: [EtcdUpgrade]
  ↓
  executeBatchParallel
    ↓
    executeComponent(Etcd)
      ↓
      InlineRunner.Execute("EnsureEtcdUpgrade")
        ↓
        Phase.Execute() → 升级 Etcd
      ↓
    markComponentCompleted(Etcd)
  ↓
Batch 3: [MasterUpgrade, WorkerUpgrade]
  ↓
  executeBatchParallel
    ↓
    并行执行 Master 和 Worker 升级
  ↓
返回 kerrors.NewAggregate(所有错误)
```

## **总结**

ExecuteDAG 是声明式升级的**核心编排引擎**：
1. **拓扑编排**：将 DAG 转换为可执行的批次序列
2. **并行优化**：批次内并行执行，提升升级效率
3. **依赖保证**：批次间串行执行，确保依赖关系
4. **错误处理**：支持 FailFast 和错误聚合
5. **断点续传**：跳过已完成组件，支持重试
6. **状态持久化**：标记组件完成/失败状态

这种设计实现了**高效、可靠、可控**的升级流程：拓扑排序 → 批次并行 → 依赖保证 → 错误处理 → 状态持久化。
...
**`ClusterApplier` 是面向目标集群的组件清单应用器，负责将组件的 YAML 清单渲染、守卫检查后提交到目标 Kubernetes 集群并等待就绪。**

### 结构体定义

[applier.go:32](file:///\cluster-api-provider-bke\pkg\manifest\applier.go#L32)：
```go
type ClusterApplier struct {
    ctx        context.Context          // 生命周期上下文
    client     client.Client            // 管理集群的 Controller Runtime 客户端
    bkeCluster *bkev1beta1.BKECluster   // 目标 BKECluster 对象（提供集群信息）
    logger     *bkev1beta1.BKELogger    // 日志记录器
    nodes      bkenode.Nodes            // 集群节点信息
}
```
它实现了 `Applier` 接口：
```go
...
ClusterApplier
  │
  ├─ ApplyComponent()          ← 唯一公开方法（接口实现），编排整个应用流程
  │     ├─ kubeClient()        ← 获取目标集群远程客户端
  │     ├─ renderParams()      ← 构建模板渲染参数
  │     ├─ guardComponentInstalled()  ← 守卫：探测组件是否已安装
  │     ├─ prepareKubeClientForApply() ← 注入日志和上下文到客户端
  │     └─ applyPackageManifests()     ← 逐个应用 YAML 清单
  │
  ├─ imageRepo()               ← 从 BKECluster 配置获取镜像仓库地址
  │
  └─ (辅助方法均被 ApplyComponent 编排调用)
```
...
| 方法 | 职责 | 关键细节 |
|------|------|---------|
| [ApplyComponent](file:///\cluster-api-provider-bke\pkg\manifest\applier.go#L61) | 编排入口 | 串联 4 个步骤，任一失败即返回 |
| [kubeClient](file:///\cluster-api-provider-bke\pkg\manifest\applier.go#L160) | 建立连接 | 通过 `kube.NewRemoteClientByBKECluster` 连接到**目标集群**（不是管理集群） |
| [renderParams](file:///\cluster-api-provider-bke\pkg\manifest\applier.go#L175) | 渲染参数 | 优先用 `impl.RenderParamsForCluster` 获取完整参数，失败则回退到 `fallbackRenderParams`（仅含 namespace） |
| [guardComponentInstalled](file:///\cluster-api-provider-bke\pkg\manifest\applier.go#L87) | 安装守卫 | 渲染清单 → 提取工作负载引用 → 检查目标集群中是否存在；**不存在则返回 `SkipNotInstalledError`** |
| [prepareKubeClientForApply](file:///\cluster-api-provider-bke\pkg\manifest\applier.go#L110) | 客户端准备 | 将 logger 和 ctx 注入到 `kube.Client` 实例中，使 ApplyYaml 过程有日志输出和上下文控制 |
| [applyPackageManifests](file:///\cluster-api-provider-bke\pkg\manifest\applier.go#L138) | 应用清单 | 逐个 YAML 文档创建 `kube.Task`，设置升级模式 + 等待就绪 + 镜像仓库替换，调用 `ApplyYaml` 提交 |
| [imageRepo](file:///\cluster-api-provider-bke\pkg\manifest\applier.go#L130) | 镜像仓库 | 从 `BKECluster.Spec.ClusterConfig` 提取镜像仓库地址，用于替换清单中的镜像前缀 |
...
Scheduler.ExecuteDAG
  └─ executeBatchParallel
       └─ executeComponent
            ├─ Inline 类型 → InlineRunner.Execute()
            └─ Manifest 类型 → executeManifest()
                                 ├─ ManifestStore.GetComponentManifests()  → 获取清单内容
                                 └─ ManifestApplier.ApplyComponent()       → ClusterApplier ← 这里
```
`ClusterApplier` 是 DAG 调度器中 **Manifest 类型组件**的唯一执行器。DAG 中每个没有 `Inline` 处理器的组件节点，最终都由 `ClusterApplier` 负责将清单应用到目标集群。

### 设计要点

1. **管理集群 vs 目标集群**：`ClusterApplier` 的 `client` 是管理集群客户端（用于读取 BKECluster 资源），而实际 YAML 通过 `kubeClient()` 建立的**目标集群连接**提交
2. **守卫模式**：`guardComponentInstalled` 确保只升级已安装的组件，避免对未部署组件执行升级操作
3. **等待就绪**：`SetWaiter(true, ...)` 确保每个清单应用后会等待资源达到就绪状态，而非 fire-and-forget
4. **镜像仓库替换**：`AddRepo(repo)` 统一处理私有化场景下的镜像地址替换
