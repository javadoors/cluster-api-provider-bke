...
# 可靠性字段revision
### 概要
简要说明 **`revision`、`installedVersion`、`lastAppliedRevision`、`capbke.io/component-revision=<revision>`** 这些字段/标签的语义与用途，以及 **`pruneOldRevisions`** 在组件生命周期管理中的作用、实现要点与最佳实践。目标是让你能在控制器设计、回滚策略与资源清理上做出一致、安全的决策。
### 字段与标签含义
- **`revision`（执行结果标识）**  
  **含义**：由执行器返回的、标识一次成功应用操作的字符串（例如 `k8s/etcd-3.5.12`、`helm/etcd-3.5.12`、`builtin/etcd-3.5.12`）。  
  **用途**：作为操作的唯一标识，用于审计、历史记录与回滚参考。  

- **`installedVersion`（已安装版本）**  
  **含义**：组件当前被认为已安装或运行的版本号（通常来自 `ComponentDescriptor.spec.version` 在成功安装后写入 status）。  
  **用途**：控制器用来判断是否需要升级；用于展示当前运行版本给运维人员。  

- **`lastAppliedRevision`（上次应用的 revision）**  
  **含义**：上一次成功应用并写入集群的 `revision` 值（通常写入 `ComponentDescriptor.status.lastAppliedRevision` 或 `PhaseManifest.status`）。  
  **用途**：回滚时的目标或参考；用于 prune/清理时识别当前有效资源集合。  

- **标签 `capbke.io/component-revision=<revision>`**  
  **含义**：在每次成功 apply 时，K8sExecutor（或其他执行器）在被管理资源上打上的标签，值为该次操作的 `revision`。  
  **用途**：把资源与某次组件发布（revision）绑定，便于后续识别、查询与清理旧版本资源。
### pruneOldRevisions 的作用与价值
- **核心目的**：清理属于同一组件但不再属于当前有效 `revision` 的 Kubernetes 资源，避免资源残留、配置冲突与资源泄露。  
- **为什么需要**：组件 manifest 可能随版本变化（资源名、类型、字段），旧资源若不清理会造成：
  - 冗余或冲突的 Service/ConfigMap/Secret；
  - 升级后仍被旧配置影响的行为；
  - 资源配额浪费与运维混乱。  
- **价值**：
  - 保持集群资源与当前组件版本一致（drift reduction）。  
  - 降低人为误操作恢复成本（回滚时可更精确地恢复到上一个 revision）。  
  - 简化审计与故障排查（通过 revision 快速定位变更集）。
### pruneOldRevisions 工作原理（实现要点）
1. **标记阶段**：每次成功 apply，执行器为所有被管理资源设置两类标签：  
   - `capbke.io/component=<component-name>`  
   - `capbke.io/component-revision=<revision>`  
2. **列出阶段**：prune 逻辑按组件名列出所有带 `capbke.io/component=<component>` 的资源（按预定义的 GVK 列表或动态发现 CRD）。  
3. **筛选阶段**：对列出的资源，比较其 `capbke.io/component-revision` 与当前 `revision`：  
   - 若相同 → 保留；若不同或缺失 → 视为旧资源候选。  
4. **删除阶段**：对候选资源执行删除（best-effort），并记录事件与日志；对关键资源可先做 dry-run 或标记为待删除以便人工确认。  
5. **安全策略**：  
   - **ownerReferences 优先**：若资源有 ownerReferences 指向当前控制对象（例如 PhaseManifest/ComponentDescriptor），优先使用 ownerReferences 做垃圾回收。  
   - **确认依赖**：删除前检查是否被其他组件引用（例如 Service 被其他组件使用），避免误删。  
   - **回退窗口**：保留短期回退窗口（例如 24–72 小时）或将删除操作分阶段执行（标记 → 删除）。
### 示例与最佳实践
**示例：成功 apply 后的标签与 status 写入**
```yaml
# ComponentDescriptor.status (片段)
status:
...
metadata:
  name: etcd-config
  labels:
    capbke.io/component: "etcd"
    capbke.io/component-revision: "k8s/etcd-3.5.12"
data:
  ...
```
**最佳实践要点**
- **Revision 格式规范**：建议包含执行器类型、组件名与版本（例如 `k8s/<name>-<version>` 或 `helm/<name>-<version>`），便于可读性与解析。  
- **写入时机**：只有在操作完全成功（包括 post-check）后才写 `lastAppliedRevision` 与在资源上打 `component-revision`。  
- **Prune 范围控制**：默认 prune 常见资源类型（Deployment/StatefulSet/Service/ConfigMap/Secret/Job），CRD 需显式列入或动态发现。  
- **Dry-run 与分阶段删除**：对生产环境先做 dry-run 或先把旧资源标记为 `prune-pending`，等待人工确认或自动延迟删除。  
- **回滚支持**：保留 `lastAppliedRevision` 与历史记录（history[]），便于在升级失败时回滚到上一个 revision（或使用 snapshot 恢复有状态组件）。  
- **审计与事件**：每次 prune 操作应生成 Event 并记录到审计日志，便于追踪谁/何时/为何触发删除。
### 迁移与运维注意
- **旧逻辑桥接**：在迁移期使用 `BuiltinExecutor`，其成功执行也应写入 `lastAppliedRevision` 与在资源上打标签，保证 prune 与回滚机制一致。  
- **演练**：在 staging 环境多次演练 prune 与回滚流程（尤其 etcd restore 场景），验证 snapshot 与 lastAppliedRevision 的可用性。  
- **保守策略**：对关键资源（例如 etcd StatefulSet、PV、PVC）默认不自动 prune，改为人工确认或特殊恢复流程。  
- **监控告警**：为 prune 操作与删除失败设置告警；为 `lastAppliedRevision` 变更设置审计告警，便于及时发现异常升级。
### 结论（要点回顾）
- **`installedVersion`**：表示当前运行的版本，用于判断是否需要升级。  
- **`lastAppliedRevision`**：记录上一次成功应用的 revision，是回滚与审计的关键。  
- **`capbke.io/component-revision=<revision>`**：把资源与某次发布绑定，便于识别与清理。  
- **`pruneOldRevisions`**：通过比较标签清理旧资源，保持集群与当前组件版本一致，但必须结合 ownerReferences、依赖检查、dry-run 与回退窗口以保证安全。  
