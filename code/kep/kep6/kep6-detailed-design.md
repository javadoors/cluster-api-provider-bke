...
YAML 组件采用与 Binary/Helm 对称的两层结构：`YamlComponentExecutor`（dagexec 调度层，见 8.3.3.3）+ `YamlInstaller`（`pkg/yamlinstaller` 引擎层，本节）。`YamlInstaller` 持有 `manifest.Applier`，负责清单 Apply + 健康检查；与 `BinaryInstaller`/`HelmInstaller` 命名对称。区别仅在 `YamlInstaller` 内部更简单——无 SSH/下载/缓存等子层，仅 Apply + healthcheck。

| 层 | Binary | Helm | YAML |
|----|--------|------|------|
| dagexec 执行器 | `BinaryComponentExecutor` | `HelmComponentExecutor` | `YamlComponentExecutor`（见 8.3.3.3） |
...
**Executor（调度层）完成的逻辑**（`YamlComponentExecutor`，见 8.3.3.3）：
...
func (i *YamlInstaller) Apply(ctx context.Context, cv *configv1alpha1.ComponentVersion, execCtx *ExecutionContext) error {
    ...
    // 复用共享 pkg/healthcheck 包 (见 5.4)，避免与 HelmInstaller 重复
...
> 健康检查实现委托共享 `pkg/healthcheck` 包（`healthcheck.Run(ctx, execCtx.TargetClient, *cv.Spec.YAML.HealthCheck)`）。共享包的接口定义与 PodReady/EndpointReady/Custom 检查实现详见 **5.4 HealthCheck 共享包设计**。

---

## 7. TemplateRenderer 详细设计

### 7.0 与现有 TemplateContext 的复用关系
...
2. `InlinePhaseRunner`（签名含 `*phaseframe.PhaseContext`）替换为 phaseframe-free 的 `InlineRunner` 接口（见 8.3.3.4），由 controllers 层 `InlinePhaseRunnerAdapter` 桥接。
...
- **各 Config 类型定义**：`BinaryInstallerConfig`（第 4 章）、`HelmInstallerConfig`（第 5 章）、`NewConfigRenderer`（第 8 章）
...
//   (见 8.3.3.4)，由 controllers 层提供适配实现
...
##### 8.3.3.3 YamlComponentExecutor (YAML)
...
> 健康检查由 `YamlInstaller.Apply` 内部委托共享 `pkg/healthcheck` 包完成（见 **5.4 HealthCheck 共享包设计**）。
...
**调用方式对齐**：原设计中 `featuregate.Enabled(featuregate.BinaryComponentSupport)` 改为 `featuregate.BinaryComponentEnabled(cluster)`（见 8.1.4 `buildSchedulerConfig`、11.3.5 `getK8sEnvInitScope`）。
...
```yaml
# bke-manifests/containerd/v1.7.18/component.yaml (简化示例，完整定义见 12.3.2)
apiVersion: config.openfuyao.cn/v1alpha1
kind: ComponentVersion
metadata:
  name: containerd-v1.7.18
