...
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  L1: StateMachineEngine (集群层状态机)                                   │   │
│  │  ─────────────────────────────────────────                               │   │
│  │  职责: 管理集群生命周期 (Pending/Installing/Running/Upgrading/...)        │   │
│  │  驱动: BKEClusterReconciler.Reconcile() 调用                            │   │
│  │  输入: BKECluster                                                       │   │
│  │  输出: ClusterLifecyclePhase                                            │   │
│  │                                                                          │   │
│  │  ┌─────────────────────────────────────────────────────────────────┐    │   │
│  │  │  集群级组件 (scope=cluster)                                      │    │   │
│  │  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐          │    │   │
│  │  │  │ coredns  │ │kube-proxy│ │  bocop   │ │  ...     │          │    │   │
│  │  │  │ Pending  │ │Installed │ │Upgrading │ │Installed │          │    │   │
│  │  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘          │    │   │
│  │  │  状态由 L1 直接驱动                                             │    │   │
│  │  └─────────────────────────────────────────────────────────────────┘    │   │
│  │                                                                          │   │
...
│  │         ▼                                                                │   │
│  │  ┌─────────────────────────────────────────────────────────────────┐    │   │
│  │  │  L2: NodeStateMachine (节点层状态机)                             │    │   │
│  │  │  ─────────────────────────────────                               │    │   │
│  │  │  职责: 管理节点生命周期 (Pending/Provisioned/Ready/Upgrading/...) │    │   │
│  │  │  驱动: L1.EvaluateClusterState() 内部遍历所有节点调用             │    │   │
│  │  │  输入: BKENode                                                  │    │   │
│  │  │  输出: NodeLifecyclePhase                                       │    │   │
│  │  │                                                                  │    │   │
│  │  │  ┌─────────────────────────────────────────────────────────┐    │    │   │
│  │  │  │  节点级组件 (scope=node)                                  │    │    │   │
│  │  │  │  ┌──────────┐ ┌──────────┐ ┌──────────┐               │    │    │   │
│  │  │  │  │ bkeagent │ │containerd│ │ kubelet  │               │    │    │   │
│  │  │  │  │Installed │ │Upgrading │ │ Pending  │               │    │    │   │
│  │  │  │  └──────────┘ └──────────┘ └──────────┘               │    │    │   │
│  │  │  │  状态由 L2 直接驱动                                     │    │    │   │
│  │  │  └─────────────────────────────────────────────────────────┘    │    │   │
│  │  │                                                                  │    │   │
...
│  │  │  │  ─────────────────────────────────                       │    │    │   │
│  │  │  │  职责: 管理组件生命周期                                   │    │    │   │
│  │  │  │       (Pending/Installing/Installed/Upgrading/...)       │    │    │   │
│  │  │  │  驱动: L1 驱动集群级组件, L2 驱动节点级组件               │    │    │   │
│  │  │  │  输入: ComponentLifecycleStatus                          │    │    │   │
│  │  │  │  输出: ComponentLifecyclePhase                           │    │    │   │
│  │  │  └─────────────────────────────────────────────────────────┘    │    │   │
│  │  └─────────────────────────────────────────────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
...
│       ▼                                                                         │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │  L1: engine.EvaluateClusterState(cluster)                               │   │
│  │                                                                          │   │
│  │  1. 评估集群状态转换规则                                                  │   │
│  │     └─ ClusterLifecyclePhase: Pending → Installing → Running → ...       │   │
│  │                                                                          │   │
│  │  2. 驱动集群级组件状态 ──────────────────────────┐                        │   │
│  │     └─ 遍历 cluster.Status.ClusterComponentStatuses                     │   │
│  │        └─ 对每个 scope=cluster 的组件调用:                              │   │
│  │                                                                          │   │
│  │  ┌─────────────────────────────────────────────────────────────────┐    │   │
│  │  │  L3: componentSM.EvaluateComponentState(comp)  ◀─── 集群级组件  │    │   │
│  │  │      └─ ComponentLifecyclePhase: Pending → Installing → ...    │    │   │
│  │  └─────────────────────────────────────────────────────────────────┘    │   │
│  │                                                                          │   │
│  │  3. 驱动节点状态 ──────────────────────────────────┐                     │   │
│  │     └─ 遍历所有 BKENode                             │                     │   │
...
│  │                                                      │                     │   │
│  │  ┌─────────────────────────────────────────────────────────────────┐    │   │
│  │  │  L2: nodeSM.EvaluateNodeState(node)  ◀─── 节点层                │    │   │
│  │  │                                                                  │    │   │
│  │  │  a. 评估节点状态转换规则                                          │    │   │
│  │  │     └─ NodeLifecyclePhase: Pending → Provisioned → Ready → ...  │    │   │
│  │  │                                                                  │    │   │
│  │  │  b. 驱动节点级组件状态 ─────────────────────┐                    │    │   │
│  │  │     └─ 遍历 node.Status.Components           │                    │    │   │
│  │  │        └─ 对每个 scope=node 的组件调用:       │                    │    │   │
│  │  │                                               │                    │    │   │
│  │  │  ┌─────────────────────────────────────────────────────────┐    │    │   │
│  │  │  │  L3: componentSM.EvaluateComponentState(comp)           │    │    │   │
│  │  │  │      └─ ComponentLifecyclePhase: Pending → Installing   │    │    │   │
│  │  │  └─────────────────────────────────────────────────────────┘    │    │   │
│  │  └─────────────────────────────────────────────────────────────────┘    │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
...
│  Reconcile #1                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ L1: EvaluateClusterState                                                │   │
│  │   Cluster: Pending → Installing (Condition: desiredVersion != "")        │   │
│  │   Action: 初始化 OperationProgress, 创建 BKENode 资源                    │   │
│  │                                                                          │   │
│  │ L2: EvaluateNodeState (遍历所有节点)                                     │   │
│  │   Node-1: Pending (无转换，等待 Agent 推送)                              │   │
│  │   Node-2: Pending (无转换，等待 Agent 推送)                              │   │
│  │   Node-3: Pending (无转换，等待 Agent 推送)                              │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Reconcile #2 (Agent 推送完成后)                                                │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ L1: EvaluateClusterState                                                │   │
│  │   Cluster: Installing (无转换，等待节点就绪)                              │   │
│  │                                                                          │   │
│  │ L2: EvaluateNodeState (遍历所有节点)                                     │   │
│  │   Node-1: Pending → Provisioned (Condition: AgentReady && EnvReady)      │   │
│  │           → Ready (Condition: 所有节点级组件 Installed)                   │   │
│  │     L3: bkeagent@node-1:    Pending → Installing → Installed            │   │
│  │     L3: containerd@node-1:  Pending → Installing → Installed            │   │
│  │     L3: kubelet@node-1:     Pending → Installing → Installed            │   │
│  │                                                                          │   │
│  │   Node-2: Pending → Provisioned → Ready (同上)                           │   │
│  │   Node-3: Pending → Provisioned → Ready (同上)                           │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Reconcile #3 (所有节点 Ready 后)                                               │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ L1: EvaluateClusterState                                                │   │
│  │   Cluster: Installing → Running (Condition: 所有节点 Ready)              │   │
│  │   Action: 设置 FinishedAt, 记录升级完成事件                              │   │
│  │                                                                          │   │
│  │ L1: 驱动集群级组件状态                                                   │   │
│  │   L3: coredns:     Pending → Installing → Installed                     │   │
│  │   L3: kube-proxy:  Pending → Installing → Installed                     │   │
│  │   L3: bocoperator: Pending → Installing → Installed                     │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  最终状态:                                                                       │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
...
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  用户设置 desiredVersion = "v2.7.0"                                             │
│                                                                                 │
│  Reconcile #1                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ L1: EvaluateClusterState                                                │   │
│  │   Cluster: Running → Upgrading (Condition: desiredVersion != current)    │   │
│  │   Action: 初始化 OperationProgress(OperationType=Upgrade)                │   │
│  │                                                                          │   │
│  │ L1: Execute() → executeUpgrade()                                        │   │
│  │   └─ 构建升级 DAG，按拓扑顺序执行节点级组件升级                           │   │
│  │                                                                          │   │
│  │ L2: EvaluateNodeState (遍历所有节点)                                     │   │
│  │   Node-1: Ready → Upgrading                                              │   │
│  │     L3: containerd@node-1: Installed → Upgrading → Installed            │   │
│  │     L3: kubelet@node-1:    Installed → Upgrading → Installed            │   │
│  │                                                                          │   │
│  │   Node-2: Ready → Upgrading                                              │   │
│  │     L3: containerd@node-2: Installed → Upgrading → Installed            │   │
│  │     L3: kubelet@node-2:    Installed → Upgrading → Installed            │   │
│  │                                                                          │   │
│  │   Node-3: Ready (等待，按批次升级)                                        │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Reconcile #2 (Batch 1 完成后)                                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ L1: EvaluateClusterState                                                │   │
│  │   Cluster: Upgrading (无转换，等待所有节点完成)                           │   │
│  │                                                                          │   │
│  │ L2: EvaluateNodeState                                                    │   │
│  │   Node-1: Upgrading → Ready (所有组件 Installed)                         │   │
│  │   Node-2: Upgrading → Ready (所有组件 Installed)                         │   │
│  │   Node-3: Ready → Upgrading (Batch 2 开始)                               │   │
│  │     L3: containerd@node-3: Installed → Upgrading → Installed            │   │
│  │     L3: kubelet@node-3:    Installed → Upgrading → Installed            │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  Reconcile #3 (所有节点完成后)                                                  │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ L1: EvaluateClusterState                                                │   │
│  │   Cluster: Upgrading → Running (Condition: 所有节点 Ready)               │   │
│  │   Action: 更新 currentVersion = "v2.7.0", 设置 FinishedAt               │   │
│  │                                                                          │   │
│  │ L1: 驱动集群级组件状态                                                   │   │
│  │   L3: coredns:    Installed → Upgrading → Installed (v2.7.0)            │   │
│  │   L3: bocoperator: Installed → Upgrading → Installed (v2.7.0)           │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
│  最终状态:                                                                       │
│  ┌─────────────────────────────────────────────────────────────────────────┐   │
│  │ L1: Cluster = Running, currentVersion = "v2.7.0"                        │   │
│  │ L2: Node-1/2/3 = Ready                                                  │   │
│  │ L3: 所有组件 = Installed (v2.7.0)                                       │   │
│  └─────────────────────────────────────────────────────────────────────────┘   │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```
...
│           ▼                                                                     │
│  ┌──────────────────────────────────────────────────────────────────────────┐  │
│  │ L1: StateMachineEngine                                                   │  │
│  │                                                                          │  │
│  │  读取 ──────────────────────────────────────────────────────────────┐    │  │
│  │         │                                                          │    │  │
│  │         ▼                                                          │    │  │
│  │  ┌─────────────────┐                                               │    │  │
│  │  │ cluster.Status  │ ◀── 写入 ClusterLifecyclePhase                │    │  │
│  │  │ .LifecyclePhase │                                               │    │  │
│  │  └─────────────────┘                                               │    │  │
│  │                                                                    │    │  │
│  │  读取 ──────────────────────────────────────────────────────┐     │    │  │
...
│  │  │         ▼                                        │   │    │     │    │  │
│  │  │  ┌─────────────────┐                             │   │    │     │    │  │
│  │  │  │ node.Status     │ ◀── 写入 NodeLifecyclePhase │   │    │     │    │  │
│  │  │  │ .LifecyclePhase │                             │   │    │     │    │  │
│  │  │  └─────────────────┘                             │   │    │     │    │  │
│  │  │                                                   │   │    │     │    │  │
│  │  │  读取 ────────────────────────────────────┐      │   │    │     │    │  │
