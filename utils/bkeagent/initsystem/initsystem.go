...
Original file: https://github.com/kubernetes/kubernetes/blob/release-1.15/pkg/util/initsystem/initsystem.go
*/
...
type InitSystem interface {
	...
	// ServiceDisable ensures the service is disabled to start on each boot.
	ServiceDisable(service string) error

	// ServiceEnable ensures the service is enabled to start on each boot.
